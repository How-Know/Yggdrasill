...
class _ProblemBankScreenState extends State<ProblemBankScreen>
  ...
  Map<String, dynamic> _removeFigureAssetFromMeta(
    ProblemBankQuestion q,
    Map<String, dynamic> asset,
    int orderHint,
  ) {
    final targetId = '${asset['id'] ?? ''}'.trim();
    final targetPath = '${asset['path'] ?? ''}'.trim();
    final targetKey = _figureScaleKeyForAsset(asset, orderHint);
    final targetIndex = int.tryParse('${asset['figure_index'] ?? ''}');
    final keysToRemove = <String>{
      if (targetKey.trim().isNotEmpty) targetKey.trim(),
      if (targetIndex != null && targetIndex > 0) 'idx:$targetIndex',
      if (targetPath.isNotEmpty) 'path:$targetPath',
      'ord:$orderHint',
    };

    final updatedAssets = _figureAssetsOf(q).where((item) {
      final sameId = targetId.isNotEmpty && '${item['id'] ?? ''}' == targetId;
      final samePath =
          targetPath.isNotEmpty && '${item['path'] ?? ''}' == targetPath;
    ...
    final updatedMeta = <String, dynamic>{...q.meta};
    if (updatedAssets.isEmpty) {
      updatedMeta
        ..remove('figure_assets')
        ..remove('figure_layout')
        ..remove('figure_render_scale')
        ..remove('figure_render_scales')
        ..remove('figure_horizontal_pairs')
        ..remove('figure_review_required')
        ..remove('figure_last_generated_at');
      updatedMeta['figure_count'] = 0;
      return updatedMeta;
    }

    updatedMeta['figure_assets'] = updatedAssets;
    updatedMeta['figure_count'] = updatedAssets.length;
    updatedMeta['figure_review_required'] =
        updatedAssets.any((item) => item['approved'] != true);

    final layout = updatedMeta['figure_layout'];
    if (layout is Map) {
      final items = layout['items'];
      final nextItems = items is List
          ? items.whereType<Map>().map((raw) {
              return raw.map((key, value) => MapEntry('$key', value));
            }).where((item) {
              final key = '${item['assetKey'] ?? ''}'.trim();
              final index = int.tryParse('${item['figureIndex'] ?? ''}');
              return !keysToRemove.contains(key) &&
                  (index == null || !keysToRemove.contains('idx:$index'));
            }).toList(growable: false)
          : const <Map<String, dynamic>>[];
      final groups = layout['groups'];
      final nextGroups = groups is List
          ? groups.whereType<Map>().map((raw) {
              return raw.map((key, value) => MapEntry('$key', value));
            }).where((group) {
              final members = group['members'];
              if (members is! List) return true;
              return !members
                  .any((member) => keysToRemove.contains('$member'.trim()));
            }).toList(growable: false)
          : const <Map<String, dynamic>>[];
      if (nextItems.isEmpty) {
        ...
        updatedMeta['figure_layout'] = <String, dynamic>{
          ...layout.map((key, value) => MapEntry('$key', value)),
          'items': nextItems,
          'groups': nextGroups,
    ...
    if (scaleMap.isNotEmpty) {
      final nextScaleMap = <String, dynamic>{};
      for (final entry in scaleMap.entries) {
        if (!keysToRemove.contains(entry.key)) {
          nextScaleMap[entry.key] = entry.value;
        }
      }
      if (nextScaleMap.isEmpty) {
        updatedMeta.remove('figure_render_scales');
      } else {
        updatedMeta['figure_render_scales'] = nextScaleMap;
      }
    }

    final pairs = updatedMeta['figure_horizontal_pairs'];
    if (pairs is List) {
      final nextPairs = pairs.whereType<Map>().where((raw) {
        final a = '${raw['a'] ?? raw['left'] ?? ''}'.trim();
        final b = '${raw['b'] ?? raw['right'] ?? ''}'.trim();
        return !keysToRemove.contains(a) && !keysToRemove.contains(b);
      }).toList(growable: false);
      if (nextPairs.isEmpty) {
        updatedMeta.remove('figure_horizontal_pairs');
      } else {
        updatedMeta['figure_horizontal_pairs'] = nextPairs;
      }
    }
  ...
  Future<void> _deleteFigureAsset(
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('${q.questionNumber}번 그림 삭제'),
        content: Text('그림 $orderHint을 삭제할까요? 이 작업은 바로 저장됩니다.'),
        actions: [
          TextButton(
  ...
  Future<void> _openReviewDialog(ProblemBankQuestion question) async {
    ...
    final blankChoiceLabelCtrls = _blankChoiceLabelsOf(question)
        ...
        .toList(growable: false);
    final updatedQuestion = await showDialog<ProblemBankQuestion>(
      context: context,
      builder: (context) {
        return AlertDialog(
          backgroundColor: _panel,
            ...
            FilledButton(
              ...
              onPressed: () async {
                try {
                  ...
                  if (context.mounted) Navigator.of(context).pop(updatedQ);
                } catch (e) {
    ...
    stemCtrl.dispose();
    noteCtrl.dispose();
    if (updatedQuestion == null || !mounted) return;
    setState(() {
      _questions = _questions
    ...
    try {
      final previewUrl = await _saveAndRefreshPreview(updatedQuestion);
      if (!mounted) return;
  ...
  Future<void> _openSetScoreDialog(ProblemBankQuestion question) async {
    ...
    if (updatedQuestion == null || !mounted) return;
    final scoreParts = updatedQuestion.scoreParts;
    final total = scoreParts != null && scoreParts.isNotEmpty
        ? sumScoreParts(scoreParts)
        : (_scorePointOf(updatedQuestion) ?? 0);
    setState(() {
      _questions = _questions
          .map((q) => q.id == question.id ? updatedQuestion : q)
          .toList(growable: false);
      // score draft 도 총점으로 동기화(일반 인풋 표시 일관성용).
      _scoreDrafts[question.id] = total > 0 ? _scorePointInputText(total) : '';
      _dirtyQuestionIds.add(question.id);
    });
    try {
      await _saveAndRefreshPreview(updatedQuestion);
    } catch (e) {
      if (!mounted) return;
      _showSnack('배점 저장·미리보기 갱신 실패: $e', error: true);
    }
  }
  ...
  List<String> _blankChoiceLabelsOf(ProblemBankQuestion q) {
    ...
    final configured = values
        ...
        .toList(growable: false);
    if (configured.length >= 2 && configured.length <= 4) {
      return configured;
    }
  ...
  Widget _buildInlineFigureVisual(
        ...
        children: [
          ...
          Positioned(
            ...
            child: Tooltip(
              message: '그림 삭제',
              child: Material(
                color: const Color(0xCC2F3138),
                shape: const CircleBorder(),
                child: InkWell(
                  customBorder: const CircleBorder(),
                  onTap: () => unawaited(
                    _deleteFigureAsset(q, asset, orderHint),
                  ),
    ...
    if (previewUrl.isNotEmpty) {
      return withDeleteButton(ClipRRect(
        borderRadius: BorderRadius.circular(4),
        child: Image.network(
          previewUrl,
          fit: BoxFit.contain,
    ...
    return withDeleteButton(Container(
      alignment: Alignment.center,
      height: figureHeight * 0.62,
      decoration: BoxDecoration(
  ...
  Widget _buildBlankChoicePreviewBlock(
    ...
    Widget cell(
      ...
      final isFigureCell =
          !header && RegExp(r'^\[(?:그림|도형|도표)\]$').hasMatch(raw);
      ...
      for (final indexedChoice in choices.take(5).toList().asMap().entries)
        Row(
          ...
          children: <Widget>[
            ...
            for (final entry in _splitBlankChoiceCells(
                    indexedChoice.value.text, labels.length)
                .asMap()
                .entries)
              cell(
                entry.value,
                entry.key,
                rowVisualScale: heightScale * rowScales[indexedChoice.key],
              ),
          ],
        ),
    ];
                    ...
                    if (hasFigureMarker) ...[
                      _buildManualFigureDropPanel(
                        ...
                        onUpload: (bytes, fileName) async {
                          ...
                          final updated = await _uploadManualFigureForQuestion(
                            q: dialogQ,
                            bytes: bytes,
                            fileName: fileName,
                          );
  ...
  String _commonStemGroupKeyOf(ProblemBankQuestion q) {
    final setKey =
        '${q.setModel['set_key'] ?? q.setModel['setKey'] ?? ''}'.trim();
  ...
  /// 독립형 세트의 PDF 배치 방식.
  ///   'auto'  : 슬롯(단)을 넘치면 [공통발문+첫문항] 머리조각 + 나머지 평문항으로 자동 분할 (기본)
  ///   'split' : 항상 분할
  ///   'join'  : 항상 한 슬롯에 통째로 (절대 분할 안 함)
  String _independentSetRenderModeOf(ProblemBankQuestion q) {
    for (final item in _commonStemGroupQuestions(q)) {
      final raw =
          (item.meta['set_render_mode'] ?? '').toString().trim().toLowerCase();
      if (raw == 'split' || raw == 'join' || raw == 'auto') return raw;
    }
    final self =
        (q.meta['set_render_mode'] ?? '').toString().trim().toLowerCase();
    if (self == 'split' || self == 'join') return self;
    return 'auto';
  }

  String _independentSetRenderModeLabel(String mode) {
    switch (mode) {
      case 'split':
        return '분할';
      case 'join':
  ...
  void _setIndependentSetRenderMode(ProblemBankQuestion q, String mode) {
    final normalized = (mode == 'split' || mode == 'join') ? mode : 'auto';
    final group = _commonStemGroupQuestions(q);
    final groupIds =
        group.isEmpty ? <String>{q.id} : group.map((item) => item.id).toSet();
    setState(() {
      _questions = _questions.map((item) {
        if (!groupIds.contains(item.id)) return item;
        final meta = Map<String, dynamic>.from(item.meta);
        if (normalized == 'auto') {
          meta.remove('set_render_mode');
        } else {
          meta['set_render_mode'] = normalized;
        }
    ...
    _showSnack(
      '세트 배치 방식을 "${_independentSetRenderModeLabel(normalized)}"(으)로 변경했습니다. '
      '저장 후 서버 PDF에 반영됩니다.',
    );
  }
  ...
  Future<ProblemBankQuestion?> _uploadManualCommonFigureForQuestion({
    ...
    try {
      ...
      final markerCount =
          _figureMarkerCountRegex.allMatches(q.setCommonStem).length;
  ...
  Future<void> _openCommonStemPreviewDialog(ProblemBankQuestion q) async {
    ...
    if (hasFigures) {
      ...
      if (mounted && signedByPath.isNotEmpty) {
        ...
        final latestPath = '${latest?['path'] ?? ''}'.trim();
        final fallbackLatestPath =
            latestPath.isNotEmpty ? latestPath : signedByPath.keys.first;
                            ...
                            _buildManualFigureDropPanel(
                              ...
                              onUpload: (bytes, fileName) async {
                                ...
                                if (updatedSource != null) {
                                  ...
                                  await refreshCommonServerPreview(
                                      commonQ, null);
                                }
                                if (dialogContext.mounted) {
                                  setLocalState(() => refreshing = false);
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      const Icon(Icons.swap_horiz, color: _textSub, size: 14),
                      const SizedBox(width: 6),
                      const Text(
                        '가로묶음 사이 간격',
                        style: TextStyle(
    ...
    for (var i = 0; i < fallbackCount; i += 1) {
      ...
      if (isImageChoiceQ) {
        if (i < stemFigCount) {
          ...
          label =
              '선지 ${ci < circledChoiceLabels.length ? circledChoiceLabels[ci] : '${ci + 1}'}';
        }
      }
      final path = '${asset['path'] ?? ''}'.trim();
  ...
  Future<void> _openAnswerReviewDialog(ProblemBankQuestion question) async {
                ...
                FilledButton(
                  ...
                  onPressed: () async {
                    try {
                      ...
                      FocusManager.instance.primaryFocus?.unfocus();
                      if (context.mounted) {
                        Navigator.of(context).pop(updatedQ);
                      }
                    } catch (e) {
    ...
    setState(() {
      _questions = _questions
          .map(
            (q) => q.id == question.id ? updatedQuestion : q,
          )
          .toList(growable: false);
    });
    try {
  ...
  /// 독립형 세트 카드의 "PDF 배치 방식" 선택 버튼.
  ///   자동(기본) / 분할 / 통짜 중 선택해 세트 전체 meta(set_render_mode)에 반영한다.
  Widget _buildSetRenderModeButton(ProblemBankQuestion q) {
    final mode = _independentSetRenderModeOf(q);
    final isOverridden = mode != 'auto';
    final color = isOverridden ? const Color(0xFF6FA8DC) : _textSub;
    return Tooltip(
      message: '세트 PDF 배치 방식\n'
          '· 자동: 단(슬롯)을 넘치면 공통발문+첫문항만 남기고 나머지를 다음 단으로 분할\n'
          '· 분할: 항상 분할\n'
          '· 통짜: 항상 한 단에 통째로',
      child: PopupMenuButton<String>(
        tooltip: '',
        initialValue: mode,
        onSelected: (value) => _setIndependentSetRenderMode(q, value),
        padding: EdgeInsets.zero,
        itemBuilder: (context) => const <PopupMenuEntry<String>>[
          PopupMenuItem<String>(
            value: 'auto',
            child: Text('자동 (슬롯 넘치면 분할)'),
          ),
          PopupMenuItem<String>(
            value: 'split',
            child: Text('항상 분할'),
        ...
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.view_agenda_outlined, size: 14, color: color),
              const SizedBox(width: 3),
              Text(
                '배치:${_independentSetRenderModeLabel(mode)}',
                style: TextStyle(
                  fontSize: 11.0,
                  fontWeight: FontWeight.w700,
                  color: color,
