...
class BackfillRunner {
  ...
  static Future<void> runAll() async {
    try {
      ...
      final only =
          const String.fromEnvironment('BACKFILL_ONLY', defaultValue: '')
              .toLowerCase()
              .trim();
      ...
      if (only.isNotEmpty) {
        if (only == 'student_time_blocks' || only == 'stb' || only == 'w5') {
          final serverStudentIds =
              await _fetchServerStudentIds(supa, academyId);
  ...
  static Future<void> _backfillTagPresets(
      SupabaseClient supa, String academyId) async {
    try {
  ...
  static Future<void> _backfillGroupsAndClasses(
      SupabaseClient supa, String academyId) async {
    try {
  ...
  static Future<void> _backfillResources(
      SupabaseClient supa, String academyId) async {
    try {
      ...
      if (folders.isNotEmpty) {
        final rows = folders.map((f) {
          ...
          folderIdSet.add(id);
          final parentId = f['parent_id'] == null
              ? null
              : _toUuid(f['parent_id'].toString(), 'resource_folders');
          return {
            'id': id,
            'academy_id': academyId,
            'name': f['name'],
  ...
  static Future<void> _backfillExamTables(
      SupabaseClient supa, String academyId) async {
    try {
      ...
      if (hours.isNotEmpty) {
        final rows = hours.map((h) {
          final breaks = h.breakTimes
              ...
              .toList();
          final id = _toUuid(
              'd${h.dayOfWeek}_${h.startHour}:${h.startMinute}-${h.endHour}:${h.endMinute}_${jsonEncode(breaks)}',
              'operating_hours');
          return {
            'id': id,
            'academy_id': academyId,
    ...
    try {
      ...
      if (rowsLocal.isNotEmpty) {
        final rows = rowsLocal.map((r) {
          ...
          String? dt(String? s) => (s == null || s.isEmpty)
              ? null
  ...
  static Future<Set<String>> _backfillW4(
      SupabaseClient supa, String academyId) async {
      ...
      if (rowsLocal.isNotEmpty) {
        ...
        final rows = rowsLocal.map((r) {
          ...
          return {
            ...
            'student_id': _toUuid(rawStudentId, 'students'),
            'registration_date':
                fromMs(r['registration_date'])?.toIso8601String(),
  ...
  static Future<void> _backfillTagEvents(
      SupabaseClient supa, String academyId) async {
    try {
      // 서버에 이미 있으면 스킵
      ...
      final rows = local.map((r) {
        ...
        final int? colorSigned = (col is int)
            ? col.toSigned(32)
            : int.tryParse(col?.toString() ?? '')?.toSigned(32);
        return {
          'academy_id': academyId,
          'set_id': r['set_id'],
          'tag_name': r['tag_name'],
  ...
  static Future<void> _backfillW5W6W7(SupabaseClient supa, String academyId,
      [Set<String>? validStudentIds]) async {
        ...
        final rows = blocks
            .map((b) {
              ...
              if (validStudentIds != null &&
                  validStudentIds.isNotEmpty &&
                  !validStudentIds.contains(sid)) {
                return null;
              }
              return {
    ...
    try {
      ...
      if (rowsLocal.isNotEmpty) {
        ...
        final rows = rowsLocal
            .map((r) {
              final sid =
                  _toUuid((r['student_id'] ?? '').toString(), 'students');
              if (validStudentIds != null &&
                  validStudentIds.isNotEmpty &&
                  !validStudentIds.contains(sid)) {
              ...
              return {
                ...
                'is_present': (r['is_present'] is int)
                    ? ((r['is_present'] as int) != 0)
                    : r['is_present'],
    ...
    // W7: payment_records
    try {
      final List<PaymentRecord> rowsLocal =
          await AcademyDbService.instance.getPaymentRecords();
      if (rowsLocal.isNotEmpty) {
        final rows = rowsLocal
            .map((r) {
              ...
              return {
                'id': _toUuid(
                    '${r.studentId}_${r.dueDate.millisecondsSinceEpoch}_${r.cycle}',
                    'payment_records'),
                'academy_id': academyId,
                'student_id': sid,
                'cycle': r.cycle,
  ...
  static Future<Set<String>> _fetchServerStudentIds(
      SupabaseClient supa, String academyId) async {
    try {
      final data =
          await supa.from('students').select('id').eq('academy_id', academyId);
  ...
  static Future<void> _backfillMisc(
      SupabaseClient supa, String academyId) async {
    // memos
    try {
      final db = await AcademyDbService.instance.db;
    ...
    try {
      ...
      if (rowsLocal.isNotEmpty) {
        ...
        await supa
            ...
            .upsert(rows, onConflict: 'academy_id,name');
      ...
      if (links.isNotEmpty) {
        final rows = links
            .map((r) => {
                  'academy_id': academyId,
                  'file_id': _toUuid(
                      (r['file_id'] ?? '').toString(), 'resource_files'),
                  'grade': r['grade'],
                  'url': r['url'],
                })
            .toList();
    ...
    try {
      ...
      if (bk.isNotEmpty) {
        ...
        debugPrint(
            '[BACKFILL][resource_file_bookmarks] inserted=${rows.length}');
      }
    } catch (e, st) {
