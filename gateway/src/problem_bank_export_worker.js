...
const RENDER_CONFIG_VERSION = 'pb_render_v103_subq_wrap_14';
// V2 (xelatex-v2) 엔진 전용 캐시 네임스페이스 (xelatex_v2/ 파이프라인용).
//   problem_bank_api.js 의 EXPORT_RENDER_CONFIG_VERSION_V2 와 반드시 동일해야
//   동일 입력 → 동일 캐시 키가 산출된다.
const RENDER_CONFIG_VERSION_V2 = 'pb_render_v2_wrapgap_halfline_01';
...
function buildRenderConfigFromJob(job) {
  ...
  return {
    ...
    previewIndependentSetCommonStem: normalizeBool(
      options.previewIndependentSetCommonStem ??
        options.preview_independent_set_common_stem,
      false,
    ),
    disableIndependentSetGrouping: normalizeBool(
      options.disableIndependentSetGrouping ??
        options.disable_independent_set_grouping,
      false,
    ),
  };
}
