...
function wrapHangulRunsForMath(input) {
 ...
 *
 * VLM 추출물에서 `\xrightarrow{(가)}` 처럼 화살표/액센트 명령의 라벨에 한글이
 * 그대로 들어오는 경우, smartTexLineCore 의 KOREAN_SEG_RE 분할 단계에서 한글만
 * 떨어져 나와 `\xrightarrow{($가$\displaystyle )}` 같은 깨진 LaTeX 가 생성되고
 * "Missing } inserted" 컴파일 오류가 난다. 분할 이전에 인자 속 한글을 \text{} 로
 * 감싸 두면 이후 protectLatexTextBlocks 가 보호해 KOREAN_SEG_RE 가 건드리지 못한다.
 *
 * - 인자에 한글이 없으면 변환하지 않는다(순수 수식 라벨은 그대로 유지).
 * - `[under]{over}` 형태의 선택 인자/필수 인자 모두 처리한다(명령당 최대 2개).
 */
...
function downgradeKoreanSplitLeftRightParens(parts) {
  if (!Array.isArray(parts) || parts.length < 3) return parts;
  for (let i = 0; i + 2 < parts.length; i += 1) {
    const leftSeg = parts[i];
    const textSeg = parts[i + 1];
    const rightSeg = parts[i + 2];
    if (leftSeg?.type !== 'math' || textSeg?.type !== 'text' || rightSeg?.type !== 'math') {
      continue;
    }
    const leftMatch = String(leftSeg.value || '').match(/\\left\s*\((\s*)$/);
    if (!leftMatch) continue;
    const right = firstUnmatchedRightParenDelimiter(rightSeg.value);
    if (!right) continue;

    // A Korean text segment can split `\left( 단, ... \right)` into separate
    // math modes. Only that compile-failing boundary case is downgraded.
    leftSeg.value = String(leftSeg.value || '').replace(/\\left\s*\((\s*)$/, `($1`);
    rightSeg.value =
      String(rightSeg.value || '').slice(0, right.index) +
      ')' +
      String(rightSeg.value || '').slice(right.index + right.length);
  }
  return parts;
}
...
function findTopLevelSetBar(body) {
  let braceDepth = 0;
  let parenDepth = 0;
  let bracketDepth = 0;
  for (let i = 0; i < body.length; i += 1) {
    const ch = body[i];
    if (ch === '\\') {
      const command = body.slice(i).match(/^\\[A-Za-z]+/);
      if (command) {
    ...
    if (ch === '{') braceDepth += 1;
      ...
      if (/\\middle\s*$/.test(body.slice(0, i))) continue;
      return i;
    }
  }
  return -1;
...
function normalizeSetBuilderBars(math) {
  const src = String(math || '');
  ...
  while (cursor < src.length) {
    ...
    if (/\\(?:left|right)\s*$/.test(src.slice(0, start))) {
      out += '\\{';
      cursor = start + 2;
      continue;
    }
    ...
    const bar = findTopLevelSetBar(body);
    if (bar < 0) {
      out += src.slice(start, end + 2);
    } else {
      const left = body.slice(0, bar).trim();
      const right = body.slice(bar + 1).trim();
      if (left && right) {
        out += `\\YggSetBuilder{${wrapHangulRunsForMath(left)}}{${wrapHangulRunsForMath(right)}}`;
      } else {
        out += src.slice(start, end + 2);
      }
    }
...
function normalizeMathSegment(mathContent) {
  ...
  out = normalizeSetBuilderBars(out);
...
function protectLatexSetBuilderBlocks(input) {
  const source = String(input || '');
  ...
  while (cursor < source.length) {
    ...
    if (block.includes('|') && /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F가-힣]/.test(block)) {
      const token = `\u0000LATEXSET${blocks.length}\u0000`;
      blocks.push(block);
      out += token;
    } else {
      out += block;
  ...
  return {
    ...
    restore(value) {
      let restored = String(value || '');
      for (let i = 0; i < blocks.length; i += 1) {
        restored = restored.replaceAll(`\u0000LATEXSET${i}\u0000`, blocks[i]);
      }
      return restored;
    },
  };
...
function smartTexLineCore(text, equations, options = {}) {
  ...
  const protectedSetBuilders = protectLatexSetBuilderBlocks(body);
  body = protectedSetBuilders.text;
  ...
  downgradeKoreanSplitLeftRightParens(parts);

  const enableVisualLineTallMathPad = options?.visualLineTallMathPad === true;
  const visualLineTallMathPadMinPrefix = Number.isFinite(Number(options?.visualLineTallMathPadMinPrefix))
    ? Math.max(0, Number(options.visualLineTallMathPadMinPrefix))
  ...
  const result = parts
    .map((seg) => {
      ...
      math = protectedSetBuilders.restore(math);
      ...
      return `${leadSp}${addBreakAroundMath ? '\\allowbreak{}' : ''}${addVisualLinePad ? '\\mtvisuallinetallpad{}' : ''}${rendered}${trailSp}`;
    })
    .join('');

  if (prefix) {
    return protectedText.restore(protectedBoxes.restore(protectedSetBuilders.restore(`$\\displaystyle ${prefix}$${result}`)));
  }
  return protectedText.restore(protectedBoxes.restore(protectedSetBuilders.restore(result)));
}
...
function renderStandaloneMathEnvironmentBlock(lines) {
  ...
  return `\\par\\noindent\\YggVtwoBaselineGuide{}\\mtlinesymbox{$\\displaystyle ${normalized}$}\\par`;
}
...
function texHasTallInlineMath(tex) {
  ...
  return /\\(?:dfrac|frac|tfrac|over|sqrt|mtsqrtpad|mtsymmathbox|mtlinesymbox|sum|prod|int|iint|iiint)\b/.test(value)
    || /\\begin\{(?:cases|array|matrix|pmatrix|bmatrix|vmatrix|Vmatrix|smallmatrix|aligned|gathered|split)\}/.test(value);
}
 ...
 */
function renderStemTextLine(sub, equations, firstPrefix = '', options = {}) {
  ...
  const renderManualMathLineBreak = (source, manualFirstPrefix = '') => {
    ...
    return lines.length > 0 ? wrapStem(renderedLines, { tallMath: texHasTallInlineMath(renderedLines) }) : '';
  };
  // 세트형 하위문항 (1), (2), ...
...
function parseStemSegments(stem, stemLineAligns = [], opts = {}) {
  ...
  for (const seg of segments) {
    ...
    for (let rawIdx = 0; rawIdx < seg.lines.length; rawIdx += 1) {
      ...
      // inline-text 마커와 주변 텍스트는 같은 라인으로 유지(줄 흐름 안 삽입).
      let lineAcc = '';
      const flushLineAcc = () => {
        if (lineAcc) {
...
function renderDecoLine(text, equations, replaceFigureMarkers = null) {
  ...
  if (/\\includegraphics/.test(withFigs)) {
    ...
    for (const seg of segments) {
      ...
      if (/\\includegraphics/.test(seg)) {
        ...
        const tex = smartTexLine(seg.trim(), equations, BOX_TEX_OPTIONS);
  ...
  if (dialogueMatch) {
    ...
    if (namePart) {
      const nameTex = smartTexLine(namePart, equations, BOX_TEX_OPTIONS);
  ...
  return smartTexLine(withFigs, equations, BOX_TEX_OPTIONS);
}
...
function unwrapLatexTextCommandsForTextMode(input) {
  const source = String(input || '');
  if (!source.includes('\\text{')) return source;
  let out = '';
  let cursor = 0;
  while (cursor < source.length) {
    const start = source.indexOf('\\text{', cursor);
    ...
    while (i < source.length && depth > 0) {
      const ch = source[i];
      if (ch === '\\') {
        content += source.slice(i, Math.min(i + 2, source.length));
        i += 2;
        continue;
      }
      if (ch === '{') {
...
// - 이미 $...$ 로 감싸진 경우: 그대로 둔다 (raw tabular 해체 경로).
// - \text{...} 는 math mode 전용 명령이므로 표 셀(text mode)에서는 풀어서 smartTexLine 에 맡긴다.
...
function renderChoicesLatex(choices, equations, layoutColumns = 1, options = {}) {
  ...
  const rawChoiceText = (c) => (typeof c === 'string' ? c : c?.text || c?.label || '');
  const isCasesChoice = (c) => /\\begin\s*\{cases\}/.test(rawChoiceText(c));
  const renderItem = (c, idx) => {
    const text = rawChoiceText(c);
  ...
  if (choices.length === 5) {
    ...
    const softFitCases = choices.map((c) => isCasesChoice(c));
    ...
    // `cases` 선지는 TeX 상 하나의 큰 박스로 측정되어 2+2+1 한 칸을 아주 조금 넘어도
    //   전체가 세로 5줄로 떨어진다. 3줄 폭을 "조금" 넘는 cases 만 3줄 배치로 유지하고,
    //   실제 출력 시 해당 셀 내부에서만 max width 로 미세 축소한다.
    const row3SoftMaxW = '\\dimexpr0.62\\linewidth\\relax';
    const row3Cell = (cell, idx) => (softFitCases[idx]
      ? `\\adjustbox{max width=${row3W}}{${cell}}`
      : cell);
    const measureCells = cells.flatMap((cell, idx) => [
      ...
      softFitCases[idx]
        ? `\\ifdim\\wd\\YggChoiceMeasureBox>${row3W} \\ifdim\\wd\\YggChoiceMeasureBox>${row3SoftMaxW} \\YggChoiceLayout=4 \\else \\ifnum\\YggChoiceLayout<3 \\YggChoiceLayout=3 \\fi\\fi\\fi%`
    ...
    const row3a = cells.slice(0, 2).map((cell, i) => `\\makebox[${row3W}][l]{${row3Cell(cell, i)}}`).join('%\n');
    const row3b = cells.slice(2, 4).map((cell, i) => `\\makebox[${row3W}][l]{${row3Cell(cell, i + 2)}}`).join('%\n');
    const row3c = `\\makebox[${row3W}][l]{${row3Cell(cells[4], 4)}}`;
    ...
    return [
      ...
      '\\YggChoiceLayout=1',
      ...measureCells,
      '\\ifnum\\YggChoiceLayout=1',
      '  \\noindent%',
      `  ${row1}%`,
      '  \\par',
      '\\else\\ifnum\\YggChoiceLayout=2',
      '  \\noindent%',
      `  ${row2Top}%`,
      '  \\par\\noindent%',
      `  ${row2Bot}%`,
...
function buildPreamble({
  ...
  if (pairAnchorSupport) {
    lines.push('\\usepackage{zref-savepos}');
    lines.push('\\newcommand{\\YggPairAnchor}[1]{\\zsavepos{#1}}');
    lines.push('\\newcommand{\\YggPairAnchorOffset}[1]{\\ifdim#1pt>0pt\\relax\\kern#1pt\\relax\\fi}');
  } else {
    lines.push('\\newcommand{\\YggPairAnchor}[1]{}');
    lines.push('\\newcommand{\\YggPairAnchorOffset}[1]{}');
  }
  lines.push('');
  ...
  if (isMock) {
    ...
    lines.push('      \\ifx\\mockTitlePageSubtitle\\empty\\else%');
...
function inlineQuestionNumberFontPt(stemSizePt = 11, inlineNumberFontPtOverride = null) {
  const override = Number(inlineNumberFontPtOverride);
  const base = Number.isFinite(override) && override > 0
    ? override
    : Math.max(7, Number(stemSizePt) || 11);
  return base + 1.0;
}

function inlineQuestionNumberLatex(qNumDisplay, {
  stemSizePt = 11,
  inlineNumberFontPtOverride = null,
} = {}) {
  const fontPt = inlineQuestionNumberFontPt(stemSizePt, inlineNumberFontPtOverride);
  const leadPt = (fontPt * 1.08).toFixed(2);
  return `{\\fontsize{${fontPt.toFixed(2)}pt}{${leadPt}pt}\\selectfont\\bfseries ${escapeLatexText(String(qNumDisplay))}.}`;
}
...
function renderOneQuestion(question, {
  ...
  const safePairAnchorId = String(pairAnchorId || '').replace(/[^A-Za-z0-9_-]/g, '');
  const safePairAnchorOffsetPt = Math.max(
    0,
    Math.min(48, Number.isFinite(Number(pairAnchorYOffsetPt)) ? Number(pairAnchorYOffsetPt) : 0),
  );
  const pairAnchorOffsetTex = safePairAnchorOffsetPt > 0
    ? `\\YggPairAnchorOffset{${safePairAnchorOffsetPt.toFixed(3)}}`
    : '';
  const pairAnchorTex = pairAnchorMeasure && safePairAnchorId
    ? `\\YggPairAnchor{${safePairAnchorId}}`
    : '';
  const pairAnchorFirstOffsetTex = sectionLabel ? pairAnchorOffsetTex : '';
  const pairAnchorNumberOffsetTex = sectionLabel ? '' : pairAnchorOffsetTex;
  const pairAnchorLabelTex = sectionLabel ? pairAnchorTex : '';
  const pairAnchorNumberTex = sectionLabel ? '' : pairAnchorTex;
  if (sectionLabel) {
  ...
  if (numberAbove && qNumDisplay) {
    ...
    const inlineNumberTex = inlineQuestionNumberLatex(qNumDisplay, {
      stemSizePt: questionFontSizePt,
      inlineNumberFontPtOverride,
    });
    parts.push(
  ...
  let emittedStemTextLineAny = false;
  ...
  for (let i = 0; i < safeSlots; i += 1) {
    ...
    if (question) {
      lines.push(renderOneQuestion(question, {
        ...
        layoutColumns: 2,
        pairAnchorId: pairAnchorIds[i] || '',
        pairAnchorYOffsetPt: Number(pairAnchorOffsetsPt[i] || 0),
        pairAnchorMeasure,
      }));
    } else {
...
function renderMockGridPageLatex(
  ...
  const leftPairAnchorIds = [];
  const rightPairAnchorIds = [];
  const leftPairAnchorOffsetsPt = [];
  const rightPairAnchorOffsetsPt = [];
  ...
  for (let r = 0; r < maxRows; r += 1) {
    ...
    const anchorL = `pair-${pageMacroPrefix}-${r}-L`;
    const anchorR = `pair-${pageMacroPrefix}-${r}-R`;
    leftPairAnchorIds.push(qL ? anchorL : '');
    rightPairAnchorIds.push(qR ? anchorR : '');
    leftPairAnchorOffsetsPt.push(pairAnchorOffsets && qL ? Number(pairAnchorOffsets[anchorL] || 0) : 0);
    rightPairAnchorOffsetsPt.push(pairAnchorOffsets && qR ? Number(pairAnchorOffsets[anchorR] || 0) : 0);
    ...
    if (labelL || labelR) {
      ...
      leftStrutMacros.push(pairAnchor2Pass ? '' : (labelL ? '' : strutSnippet));
      rightStrutMacros.push(pairAnchor2Pass ? '' : (labelR ? '' : strutSnippet));
    ...
    //   "첫 hbox top edge" 가 수평으로 맞춰진다.
    //   depth 는 복사하지 않는다. 반대쪽에 분수가 없을 때 아래 줄간격까지 커지는 것을 막기 위함.
    ...
    const strutSnippet = pairAnchor2Pass
      ? ''
      : `{\\setbox0=\\hbox{\\csname ${macroL}\\endcsname\\csname ${macroR}\\endcsname}\\rule{0pt}{\\ht0}}`;
  ...
  const buildColumnsBlock = (heightMacro, leftHeightMacro, rightHeightMacro) => [
    ...
    renderMockSlotColumnBody(leftQuestions, safeLeftSlots, leftHeightMacro, {
      ...
      pairAnchorIds: leftPairAnchorIds,
      pairAnchorOffsetsPt: leftPairAnchorOffsetsPt,
      pairAnchorMeasure,
    ...
    renderMockSlotColumnBody(rightQuestions, safeRightSlots, rightHeightMacro, {
      ...
      pairAnchorIds: rightPairAnchorIds,
      pairAnchorOffsetsPt: rightPairAnchorOffsetsPt,
      pairAnchorMeasure,
