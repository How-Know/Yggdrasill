...
export function buildPrompt({
  ...
  const wonriLines =
    scopeSeries === 'wonri'
      ...
      : [];
  // 개념+유형(gaeyu) 전용 규칙. 코너마다 번호가 1번부터 다시 시작하고 개념확인은
  // 지면에 번호가 아예 없어서, Stage 1 이 "코너 이름 + 순번" 으로 번호를 붙인다.
  // 이 규칙을 알려주지 않으면 모델이 지면에 보이는 숫자만 적어 기대 목록과
  // 대응이 끊긴다.
  const gaeyuCornerLines = {
    A: [
      '이번 소단원은 "개념확인" 이다. 개념 설명·개념 카드는 추출 대상이 아니고,',
      '"개념확인" 이라고 붙은 문항만 추출하라.',
      '개념확인 상자 안의 (1)~(6), ㄱ~ㄹ, ①~⑤는 한 문항의 소문항·보기다.',
      '이를 "개념확인1"~"개념확인6" 같은 여러 문항으로 쪼개지 말고 상자 전체를',
      '기대 목록의 "개념확인<본문쪽>" 한 문항으로 반환하라.',
    ],
    B: [
      '이번 소단원은 "필수 문제" 다. 필수 문제와 그 아래 딸린 따름 문제(1-1, 1-2)를',
      '각각 개별 문항으로 추출하라. 유형 제목 줄과 개념 요약 박스는 문항이 아니다.',
    ],
    C: ['이번 소단원은 "쏙쏙 개념 익히기" 다. 이 코너의 문항만 추출하라.'],
    D: [
      '이번 소단원은 "탄탄 단원 다지기" 다. STEP 헤더 문구와 "중요" 배지는 문항이',
      '아니다. 번호가 붙은 문항만 추출하라.',
    ],
    E: [
      '이번 소단원은 "쓱쓱 서술형 완성하기" 다. 예제·유제·"연습해 보자" 가 섞여 있다.',
      '예제는 문제 아래에 풀이 과정과 답이 인쇄돼 있다. 그 풀이는 stem 에 넣지 말고',
      'answer 에 채워라. 유제·연습해 보자의 "풀이 과정을 쓰는 빈칸" 도 stem 이 아니다.',
    ],
    F: ['이번 소단원은 "한 번 더 연습" 이다. 이 코너의 문항만 추출하라.'],
  };
  const gaeyuLines =
    scopeSeries === 'gaeyu'
      ? [
          '=== 개념+유형 교재 전용 규칙 ===',
          '이 교재는 개념서(개념+유형)다. 한 페이지에 개념 설명, 개념확인, 필수 문제,',
          '따름 문제가 섞여 있다. 파란 배경 "교과서 +α" 카드와 노란 개념 카드는 번호가',
          '적혀 있어도 개념 설명이지 문항이 아니다.',
          '',
          '아래 기대 문항번호 목록의 번호는 코너 이름과 순번을 합쳐 우리가 붙인 것이라',
          '지면에 인쇄된 숫자와 다를 수 있다. question_number 는 지면 숫자가 아니라',
          '**목록에 적힌 번호를 그대로** 적어라.',
          '  - "개념확인8" = 본문 8쪽의 개념확인 문항. 지면에 번호가 인쇄돼 있지 않다.',
          '  - "예제1" / "유제1" / "연습1" = 쓱쓱 서술형 완성하기의 예제·유제·연습해 보자 1번.',
          '  - "1-1", "1-2" = 필수 문제 1번에 딸린 따름 문제. 대표 문항과 별개 문항이다.',
          '대표 번호 아래 (1), (2)처럼 독립 번호가 없는 소문항이 여러 개면 한 문항의',
          '종속형 세트다. is_set_question=true, set_type="dependent_set" 으로 두고',
          'sub_questions에 모두 담아라. independent_set 으로 만들지 마라.',
          ...(gaeyuCornerLines[scopeSubKey] ?? []),
          '',
        ]
      : [];
  // 수력충전 전용 규칙. 개념 설명이 문항 사이에 자주 끼고, 개념 체크는 빈칸을
  // 채우는 문항이라 개념 설명으로 오인되기 쉽다. 번호는 "01" 처럼 두 자리로
  // 인쇄된다.
  const suryeokLines =
    scopeSeries === 'suryeok'
      ? [
          '=== 수력충전 교재 전용 규칙 ===',
          '문항번호는 "01", "02" 처럼 두 자리로 인쇄된다. question_number 는 기대',
          '목록에 적힌 두 자리 형식을 그대로 적어라("1" 로 줄이지 마라).',
          '노란/연회색 개념 설명 상자와 "유형 01 …" 유형 제목 줄은 문항이 아니다.',
          '초록 "개념 체크" 배지가 붙은 둥근 상자는 빈칸을 채우는 **문항이다**.',
          '  - 상자 전체를 한 문항으로 담고, 안의 (1), (2) 는 소문항으로 둔다.',
          '  - 빈칸 [ ] 은 지우지 말고 stem 에 그대로 남겨라.',
          '독립형 세트 범위 안의 개별 문항 자체에 다시 1), 2), 3) 하위문항이',
          '붙을 수 있다. 특히 좌단 아래에서 시작한 문항의 2), 3)이 우단 위로',
          '이어질 수 있으므로 페이지 양쪽 단을 끝까지 확인해 sub_questions에 모두',
          '담아라. 이 경우 바깥 세트는 독립형이고 안쪽은 종속형이므로',
          'is_set_question=true, set_type="mixed_set"으로 둔다.',
          ...(scopeSubKey === 'B'
            ? /실력\s*향상\s*테스트/.test(
                `${scope?.sub_name || ''} ${scope?.mid_name || ''} ${scope?.big_name || ''}`,
              )
              ? [
                  '이번 소단원은 "학교 시험 대비 실력 향상 테스트" 다. 문항 위의',
                  '"시험에 꼭!", "도전해 얍!" 은 유형 배지이지 문항의 일부가 아니다.',
                  'stem 에 넣지 마라.',
                ]
              : [
                '이번 소단원은 "단원 마무리 평가" 다. 문항 위의 "계산 조심",',
                '"생각 더하기", "조건 확인" 은 유형 배지이지 문항의 일부가 아니다.',
                'stem 에 넣지 마라.',
              ]
            : [
                '이번 소단원은 유형 문제와 개념 체크다. 번호가 붙은 문항만 추출하라.',
              ]),
          '',
        ]
      : [];
  ...
  const independentSetLines = independentSetRanges.length > 0
    ? [
        ...
        '단, 범위 안의 개별 문항 하나에 다시 (1), (2), (3) 하위문항이 인쇄되어',
        '그 하위문항끼리 이어지는 구조라면 예외다. 그 문항은 sub_questions에',
        '하위문항을 전부 담고 set_type="mixed_set"으로 둔다.',
  ...
  return [
    ...
    '      "set_type": "independent_set" | "dependent_set" | "mixed_set" | null,',
    ...
    '      },',
    '      "score": <실수 또는 null>,',
    '      "figures": [ { "order": 1, "description": "그림 간단 설명", "bbox_1k": [ymin, xmin, ymax, xmax] } ],',
    ...
    '     또한 figures[] 에 각 그림의 bbox_1k 를 반드시 넣어라. bbox_1k 는 해당 PDF 페이지 이미지 기준 0~1000 정규화 좌표 [ymin, xmin, ymax, xmax] 이다.',
    '     bbox_1k 는 문제 번호/본문/선택지를 제외하고 실제 그림·도형 영역만 감싸되, 라벨·눈금·길이 표시처럼 그림 일부인 텍스트는 포함한다.',
    ...
    '     매우 중요: 도형 이름 앞의 사각형 기호(예: "□ABCD", "□ABFE", "□CDEF")는 값을 채우는 빈칸이 아니다. stem 에는 "\\square ABCD" 또는 "\\square ABFE" 처럼 쓰고 box{~~} 로 바꾸지 마라.',
    '     반대로 수식 중 값을 채워 넣는 네모칸(예: "(□)(10y+z)", "100(□+□)")은 반드시 "box{~~}" 로 적어라.',
    ...
    '[S6] 일반 줄바꿈은 JSON 표준 이스케이프 "\\n" 으로 표기하고, 문단 여백이 필요할 때만 "[문단]" 마커를 쓴다.',
    '     매우 중요: stem/choice/sub_question 의 일반 줄바꿈에 LaTeX "\\\\" 명령을 쓰지 마라.',
    '     박스 안 수식 여러 줄, 보기 여러 줄, 조건 여러 줄도 모두 "\\n" 또는 "[문단]" 으로 나눈다.',
    '     단, LaTeX 행 환경 내부 행 구분은 예외다. \\begin{tabular}, \\begin{cases}, \\begin{array}, \\begin{aligned}, \\begin{matrix} 내부에서는 반드시 "\\\\" 를 사용한다.',
    '     예: "\\begin{cases} x=1 \\\\ y=2 \\end{cases}" / 잘못된 예: "\\begin{cases} x=1 \\ y=2 \\end{cases}"',
    '[S7] 보기 박스 (<보기> 또는 <보 기>) 는 "[보기시작]" ... "[보기끝]" 사이에 본문을 그대로 담는다.',
    '     조건·규칙 박스(보기 아님)는 "[박스시작]" ... "[박스끝]" 사이에 담는다. 두 마커를 혼동하지 말 것.',
    '[S7-1] PDF 원문에서 본문 사이에 별도 줄로 들여쓰기된 "수식 제시줄" 이 명확히 보일 때만 그 줄 맨 앞에 "[수식제시]" 를 붙인다.',
    '       단순히 수식만 있는 줄이라고 자동으로 붙이지 마라. PDF 상에서 실제로 줄바꿈되어 있고, 앞뒤에 본문/수식 줄이 있으며, 해당 수식 줄이 들여쓰기되어 보이는 경우에만 사용한다.',
    '       렌더러는 "[수식제시]" 마커가 있는 줄에만 수식 제시줄 스타일을 적용하므로, 확실하지 않으면 마커를 붙이지 마라.',
    '[S7-2] 행렬 단원에서는 대괄호/소괄호로 둘러싸인 숫자 배열을 표나 그림으로 오해하지 말고 LaTeX 행렬 수식으로 추출한다.',
    '       예: "[1 2; 3 4]" 형태는 "\\\\begin{bmatrix}1 & 2 \\\\\\\\ 3 & 4\\\\end{bmatrix}" 로, "(1 2; 3 4)" 형태는 "\\\\begin{pmatrix}1 & 2 \\\\\\\\ 3 & 4\\\\end{pmatrix}" 로 쓴다.',
    ...
    '[S9-1] 객관식 선택지가 그림/그래프 자체인 "그림 선지형"이면 choices[i].text 는 "[그림]" 으로 두고, 각 선지 그림을 figures[] 에 순서대로 넣는다.',
    '       choices 에 "[그림]" 이 5개 있으면 figures[] 에도 본문 그림을 제외한 선지 그림 5개가 반드시 있어야 한다. 누락하면 안 된다.',
    '       이때 PDF 에 인쇄된 선택지 번호 ①②③④⑤ 와 그 원형 테두리는 그림이 아니라 선택지 라벨이다. figures[].bbox_1k 에 절대로 포함하지 마라.',
    '       특히 왼쪽 경계(xmin)는 번호 원의 오른쪽이 아니라 실제 그래프/도형/좌표축/그림 본체가 시작되는 지점에 맞춰 타이트하게 잡는다.',
    '       번호 원이 일부만 잘려 들어가는 crop 은 최악이다. 번호 원을 포함할 거면 전체를 포함하지 말고, 원칙적으로 완전히 제외한다.',
    '       축 이름 x,y, 원점 O, 눈금, 그래프 안 라벨처럼 그림 본체에 속한 문자는 포함한다.',
    '[S9-2] <보기> 안에 ㄱ, ㄴ, ㄷ 또는 (ㄱ), (ㄴ), (ㄷ) 로 나열된 작은 그림/그래프가 있는 경우는 ①~⑤ 객관식 그림선지가 아니다.',
    '       이 경우 choices 를 만들지 말고, 보기 전체를 stem 안의 [보기시작] ... [보기끝] 구조로 유지한다.',
    '       보기 내부의 ㄱ/ㄴ/ㄷ 그림들은 가능하면 하나의 보기 그림 영역으로 묶어 "[그림]" 하나와 figures[] 한 항목으로 추출한다.',
    '       단, 본문 오른쪽 그림처럼 보기와 별개의 그림이 있으면 본문 그림 1개 + 보기 그림 1개로 분리한다.',
    ...
    '[E2] 일반 줄바꿈은 JSON 문자열 안에서 "\\n" 이스케이프로 작성하라. "\\\\n" 같은 두 글자 리터럴이나 LaTeX "\\\\" 를 일반 줄바꿈 용도로 쓰지 마라.',
    '',
