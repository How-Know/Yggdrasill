...
// "연습해 보자" 의 저장 라벨은 "한 번 더 연습" 과 헷갈리지 않게 "서술형 연습" 이다.
// 모델은 지면에 인쇄된 대로 "연습" 을 내보내고 normalizeDifficultyLabel 이 바꾼다.
const GAEYU_LABELS = Object.freeze([
  '상',
  '중',
  '하',
  '예제',
...
export function buildRpmSetHeaderPrompt({ rawPage, displayPage }) {
  return [
    '당신은 한국 수학 교재 RPM의 "교과서문제 정복하기" 문제 페이지에서',
    '독립형 세트의 공통 지문 헤더와, 별도 보기/조건 상자가 달린 복합 문항을 찾는 비전 AI다.',
    '아래 두 대상 외의 일반 개별 문항은 추출하지 말고, 반드시 JSON만 출력한다.',
    '',
    `현재 PDF raw page=${rawPage}, display page=${displayPage}.`,
    '',
    '=== 대상 1: 독립형 세트 헤더 ===',
    '- 초록색 대괄호 범위 표기와 그 오른쪽 공통 지문을 찾는다.',
    '- 실제 예: "[0113~0116] 오른쪽 그림에서 다음을 구하시오."',
    '- 실제 예: "[0124~0127] 오른쪽 그림과 같은 사다리꼴 ABCD에 대하여 다음 설명이 옳으면 ○, 옳지 않으면 ×를 ( ) 안에 써넣으시오."',
    '- 이 범위 표기는 아래 개별 문항 0113, 0114처럼 별도의 독립형 세트 헤더다.',
    '- 대괄호가 있더라도 number에는 대괄호를 빼고 "0113~0116"처럼 기록한다.',
    '- 페이지에 보이는 모든 범위 헤더를 빠짐없이 추출한다.',
    '',
    '=== 대상 2: 보기/조건 상자가 달린 개별 문항 ===',
    '- 4자리 문항번호의 질문 바로 아래에 "보기" 상자, 조건 상자, 표 또는 큰 그림이 이어지는 문항만 찾는다.',
    '- 예: "보기에서 옳은 것만을 있는 대로 고른 것은?" 아래에 초록 테두리 보기 상자가 붙은 문항.',
    '- number는 인쇄된 4자리 번호(예: "0009"), is_set_header=false, set_range=null로 기록한다.',
    '- item_region은 질문 문장과 아래 보기/조건 상자·표·그림을 하나도 자르지 않고 모두 감싼다.',
    '- 질문 한 줄만 감싸고 아래 보기 상자를 빼면 실패다.',
    '- 일반 한 줄 계산 문항처럼 아래 부속 내용이 없는 문항은 이 보조 탐지의 items에 넣지 않는다.',
    '',
    '=== 좌표 규칙 ===',
    '- bbox: 초록색 범위 숫자만 감싼다.',
    '- 세트 헤더 item_region: 범위 숫자 오른쪽의 공통 지문과 그 지문에 딸린 공통 그림/도형 전체를 감싼다.',
    '- 복합 개별 문항 item_region: 질문 문장 시작부터 아래 보기/조건 상자·표·그림의 맨 아래까지 감싼다.',
    '- 아래쪽 개별 문항 행(0113, 0114 등)은 item_region에 포함하지 않는다.',
    ...
    '  "items": [',
    '    {',
    '      "number": "0113~0116" | "0009",',
    '      "label": "",',
    '      "is_set_header": true | false,',
    '      "set_range": {"from": 113, "to": 116} | null,',
    ...
    '}',
    '',
    '세트 헤더와 복합 개별 문항이 모두 없으면 items=[]로 반환한다.',
  ].join('\n');
}
...
// 드물게 있다. 그러면 크롭할 영역이 없어 그 지면 전체가 실패한다. 이때 번호
// 목록을 알려주고 좌표만 다시 묻는다. 어떤 문항을 뽑을지 판단은 이미 끝났으니
// 카테고리·유형명 규칙은 넣지 않고 좌표 규칙만 남긴다.
export function buildItemGeometryRepairPrompt({
  rawPage,
  displayPage,
  numbers = [],
}) {
  const list = numbers
    ...
    .filter((value) => value !== '""');
  return [
    '당신은 한국 수학 교재 스캔본에서 **이미 확정된 문항들의 좌표만** 다시 읽는',
    '비전 AI다. 문항을 새로 찾거나 빼지 말고, 반드시 JSON만 출력한다.',
    '',
    `이 이미지는 교재 스캔본의 ${displayPage ?? rawPage}페이지 (PDF raw page ${rawPage}) 이다.`,
    '',
    '=== 좌표를 찾아야 하는 문항 번호 ===',
    list.length ? list.join(', ') : '(지면에 보이는 모든 문항 번호)',
    '이 번호들을 **하나도 빠뜨리지 말고** 그대로 items 에 담아라. 번호 문자열은',
    '위에 적힌 것을 그대로 쓴다. "예제1"/"유제1" 처럼 갈래가 붙은 번호는 지면의',
    '"예제 1"/"유제 1" 배지를 가리킨다.',
    '',
    '=== 좌표 규칙 ===',
    '- 좌표계: 이미지 좌상단 (0,0), 우하단 (1000,1000). [ymin, xmin, ymax, xmax].',
    '- bbox: 문항 번호 글자(와 바로 붙은 배지)만 감싸는 최소 박스.',
    '- item_region: 그 문항의 본문만 타이트하게 감싼다. 지문·보기 상자·조건',
    '  상자·표·그림·선택지(①~⑤)·소문항 "(1)/(2)" 는 포함하고, 번호 배지와',
    '  여백의 도움말 상자, 이웃 문항, 코너 헤더는 제외한다.',
    '- 풀이를 적는 빈 칸("풀이 과정", "1단계/2단계/3단계", 맨 아래 "답" 줄)은',
    '  본문이 아니다. item_region 아래 경계는 물음과 딸린 표·그림이 끝나는 지점,',
    '  즉 "풀이 과정" 글자가 시작되기 직전이다.',
    '- 물음이 두 줄 이상이면 **마지막 줄까지** 포함한다. 아래 경계를 첫 줄에서',
    '  끊으면 문장이 잘려 못 쓰는 크롭이 된다. 각 변에 6~12 정도 여백을 둔다.',
    '- 두 문항의 item_region 은 서로 겹치지 않아야 한다.',
    '- 네 값 모두 반드시 채운다. 확실하지 않아도 비워 두지 말고 최선의 박스를',
    '  넣어라. 좌표가 없으면 그 문항은 저장되지 못한다.',
    '',
    '=== 출력 스키마 ===',
    '{',
    '  "items": [',
...
export function buildWonriPageClassPrompt({ rawPage, displayPage }) {
  return [
    ...
    '   - 개념 설명 안의 예제 제목 "확인하기"는 학습 내용을 확인하는 개념 예제명이다.',
    '     정확한 섹션 헤더 "확인 체크"와 전혀 다르며, "확인하기"만 보이는 페이지는',
    '     확인 체크 문제 페이지로 판정하지 말고 "concept"로 판정하라.',
    ...
    '- "확인하기"를 "확인 체크"의 표기 변형으로 해석하는 것은 절대 금지한다.',
    '',
...
const SERIES_CONFIGS = Object.freeze({
  ...
  // 수력충전(문제집). 소단원 행 하나에 코너가 하나뿐이라 전용 빌더로 분기한다.
  suryeok: {
    key: 'suryeok',
    bookName: '수력충전',
    partA: '유형 문제',
...
// 수력충전 문항 카테고리. concept_check 는 개념 체크(빈칸 채우기)로, 번호열이
// 유형 문제와 이어져 있어 슬롯은 A(type_problem)와 같이 쓰고 이름만 구분한다.
export const SURYEOK_ITEM_CATEGORIES = Object.freeze([
  'type_problem',
  'concept_check',
  'unit_review',
]);
...
// 수력충전 슬롯(sub_key) → 탐지 섹션 매핑.
export const SURYEOK_SECTION_BY_SUB_KEY = Object.freeze({
  A: 'type_problem',
  B: 'unit_review',
});
...
export function buildDetectProblemsPrompt({
  ...
  let lines = [
    ...
    ...(cfg.key === 'rpm'
      ? [
          '    - RPM A는 "개념 설명 1페이지 → 교과서문제 정복하기 문제 1페이지"가 교대로 반복된다.',
          '      A 범위 안에서 개념 페이지만 두 페이지 연속으로 나오지 않는다.',
          '    - 문제 페이지에는 짧은 가로형뿐 아니라 세로형·독립형 세트 문항도 있으므로',
          '      문항 높이나 번호-본문 배치가 짧은 행 모양이 아니라는 이유로 제외하지 마라.',
          '    - 굵은 4자리 문항번호가 실제로 보이면 반드시 problem_page로 두고 모두 추출한다.',
        ]
      : [
    ...
    `[D2-Type] B ${cfg.partB} / C ${cfg.partC} 의 "유형 10" 배지 숫자는 문항번호가 아니다.`,
    '     - 배지 숫자는 content_group.label 에만 넣고 item 으로 만들지 마라.',
    '     - 이 구간의 문항번호는 앞 문항에서 1씩 이어진다. 0313 다음에 "10" 같은 번호가 나올 수 없다.',
    '     - 유형 배지 오른쪽/아래의 유형명·설명 줄도 문항이 아니다.',
    ...
    ...(cfg.key === 'rpm'
      ? [
          ...
          '     - 질문 문장 바로 아래에 초록 테두리의 "보기" 상자, 조건 상자, 표 또는 그림이 이어지면 모두 같은 문항 본문이다.',
          '     - 이때 item_region의 ymax를 질문 첫 줄에서 끝내지 말고, 보기/조건 상자의 맨 아래까지 반드시 확장한다.',
          '     - 특히 "보기에서 옳은 것만을 있는 대로 고른 것은?" 같은 문장은 아래 보기 상자가 없으면 불완전한 문항이므로 보기 누락을 절대 허용하지 않는다.',
        ]
      : []),
    ...
    ...(cfg.key === 'rpm'
      ? [
          '           · 단, RPM A에서 본문이 다음 줄의 보기/조건/표/그림으로 이어지면 가로 한 줄형으로 간주하지 말고 ymax를 그 부속 내용의 끝까지 내린다.',
          '           · 아래 부속 내용이 번호보다 왼쪽에서 시작하면 xmin도 해당 상자가 잘리지 않도록 왼쪽으로 확장해도 된다.',
        ]
      : []),
...
function buildWonriDetectPrompt({ displayPage, rawPage }) {
  ...
  const lines = [
    ...
    '      개념 예제 제목 "확인하기"가 붙은 번호·예제도 문제은행 문항이 아니다.',
    '      "확인하기"는 "확인 체크"의 줄임말이나 변형이 아니므로 category="check"로',
    '      수집하지 말고 개념 페이지의 설명·예제로 취급한다.',
    ...
    '  (5) 특강 — "특강" 배지가 붙은 심화 코너. 페이지가 두 종류다:',
    '      (5a) 특강 개념 페이지 — 상단에 "특강" 배지 + 제목만 있고(배지에 번호 없음),',
    '           개념 설명·사각 박스 개념 번호(1, 2 같은 한 자리 수)·"보기" 단락이 이어진다.',
    '           **문항이 전혀 없다.** 개념 번호·"보기"를 문항으로 넣지 마라.',
    '           → page_kind="concept_page", items=[] (하단에 확인 체크가 없으면).',
    '      (5b) 특강 예제 — "특강 01" 처럼 배지에 **2자리 번호가 함께** 인쇄되고,',
    '           문제 본문과 그 아래 "풀이" 단락이 이어진다 (필수유형과 같은 구성).',
    '           번호는 01부터 새로 시작해 필수유형 번호와 별개다.',
    '      → 특강 예제(5b만): category="special_lecture", number=배지 번호 원문(예: "01"),',
    '        label="특강". 번호 오른쪽 같은 줄의 제목을 content_group.title 에 담아라',
    '        (kind="type", label="특강 01", title="그 제목").',
    '        배지에 번호가 없거나 아래 "풀이" 단락이 없으면 특강 예제가 아니다.',
    ...
    '  "page_kind": "problem_page" | "concept_page" | "mixed" | "unknown",',
    '  "concept_drill_header_visible": <bool>,',
    '  "page_layout": "two_column" | "one_column" | "unknown",',
    '  "items": [',
    '    {',
    ...
    '     - "특강 01" 처럼 배지에 번호가 있고 아래 "풀이" 단락이 있는 예제만',
    '       → "special_lecture" (label="특강"). 절대 type_example 로 분류하지 마라.',
    '       배지 없이 본문에 인쇄된 사각 박스 숫자(특히 한 자리 수)는 개념 번호이지',
    '       문항이 아니다. "풀이" 단락이 따라오지 않는 항목도 문항이 아니다.',
    '     - 정확히 "확인 체크"라고 인쇄된 독립 섹션 헤더 아래 문항만 → "check".',
    '       "확인하기"라는 예제 제목은 모양이 비슷해도 확인 체크가 아니며 문항으로',
    '       추출하지 마라.',
    '     - STEP1/STEP2/실력 UP/수능·평가원·교육청 기출 구간 문항 → "exercise".',
    '[D0-Gate] concept_drill_header_visible 은 이 페이지에 정확한 인쇄 문구',
    '     "개념원리 익히기"가 실제로 보일 때만 true 다.',
    '     "개념원리 이해", "개념원리 4", 개념 번호, 예제 번호, 이전/다음 페이지의',
...
// 수력충전 단일 패스 프롬프트.
//
// 한 지면에 코너가 하나뿐이라 개념+유형처럼 코너를 가려낼 일이 거의 없다.
// 대신 이 교재에서 실제로 어려운 두 가지에 집중한다:
//   (1) 지면 위쪽 개념 정리 박스의 번호(①②, (1)(2))를 문항으로 올리지 않기.
//   (2) 2단 조판에 20~30개씩 깔리는 짧은 문항을 하나도 빠뜨리지 않기.
function buildSuryeokDetectPrompt({ displayPage, rawPage, sectionHint = '' }) {
  const pageLine =
    displayPage != null && Number.isFinite(displayPage)
  ...
  const hintLines =
    isSkillTest
      ...
      ? [
          ...
          'category="unit_review" 다. 유형 배지가 없으니 찾지 마라.',
          '',
        ]
      : sectionHint === 'type_problem'
        ? [
            '=== 이 지면의 코너 ===',
            '지금 분석 중인 범위는 **소단원 본문(유형 문제)** 이다. 이 지면의 문항은',
            'category="type_problem" 이거나, 개념 체크 배지가 붙은',
            'category="concept_check" 둘 중 하나다. unit_review 는 나오지 않는다.',
            '',
          ]
        : [];

  const lines = [
    '당신은 한국 고등 교재 스캔본에서 "문항 번호" 의 위치를 탐지하는 비전 AI 입니다.',
    '반드시 아래 JSON 스키마만 출력하세요. 설명·마크다운·주석·코드펜스 모두 금지.',
    '',
    pageLine,
    '',
    ...hintLines,
    '=== 책 구조 (매우 중요) ===',
    '수력충전은 소단원이 아주 잘게 쪼개진 연습 위주 교재다. 지면은 대부분 2단이고',
    '한 지면에 짧은 문항이 10~30개씩 깔린다. 코너는 아래 둘뿐이다.',
    '',
    '  (1) 소단원 본문 — 지면 왼쪽 위에 파란 두 자리 번호("01")와 소단원 제목,',
    '      오른쪽 위에 "I -1 지수" 같은 단원 머리말이 인쇄된다.',
    '      · 소단원 첫 지면 맨 위에는 **하늘색/청록색 둥근 테두리 개념 정리 박스**가',
    '        있다. 그 안의 "(1) 거듭제곱", "① a^m a^n=a^(m+n)", "1 2 3 4 5" 같은',
    '        번호는 전부 개념 설명의 항목 번호다. **절대 items 에 넣지 마라.**',
    '      · 개념 박스 아래로 분홍색 "유형 01" 배지 + 유형명("지수법칙의 표현")이',
    '        나오고, 그 아래에 문항들이 이어진다. 배지 자체는 문항이 아니다.',
    '      · 문항 번호는 **초록색 굵은 두 자리 숫자**("01", "02" … "28")이고 번호',
    '        오른쪽에 본문이 바로 시작한다. → category="type_problem".',
    '        초록색 두 자리 번호가 개념 박스 바깥에 보이면 그 위에 도움말·풀이 팁·',
    '        파란 설명 배지가 길게 이어져도 무조건 새 문항이다. 도움말 때문에 번호를',
    '        개념 설명의 일부로 오인하지 마라.',
    '      · 문항 번호는 소단원마다 01부터 다시 시작해 그 소단원이 끝날 때까지',
    '        1씩 커진다. 지면이 바뀌어도 이어진다(왼쪽 단 아래 → 오른쪽 단 위).',
    '  (2) 개념 체크 — 문항 번호 **바로 위**에 초록 테두리 둥근 사각형 배지로',
    '      "개념 체크" 라고 인쇄된다. 빈칸("[    ]")을 채워 넣는 정리 문항이고',
    '      보통 지면의 **마지막 문항**으로 한 개만 붙는다.',
    '      → category="concept_check". **번호는 유형 문제 번호열을 그대로 이어받는다**',
    '      (앞 문항이 27이면 개념 체크는 28). 인쇄된 번호를 그대로 넣어라.',
    '  (3) 단원 마무리 평가 — 지면 맨 위에 "학교 시험 기본 문제 단원 마무리 평가"',
    '      리본과 오른쪽에 다루는 소단원 범위가 인쇄된다. 문항 번호는 **파란 굵은',
    '      두 자리 숫자**이고 01부터 다시 시작한다. 객관식(①~⑤)이 많다.',
    '      → category="unit_review".',
    '      · 일부 문항은 번호 **바로 오른쪽 같은 줄**에 작은 색 배지가 붙는다.',
    '        문구는 "계산 조심", "생각 더하기", "조건 확인" 중 하나 이상이고,',
    '        교재에 따라 초록 글씨 + 체크 아이콘이거나 주황 알약 모양이며',
    '        "조건 확인!" 처럼 느낌표가 붙기도 한다. 색·모양이 무엇이든 이 세',
    '        문구가 보이면 그대로 label 에 적어라(느낌표는 빼도 된다). 한 문항에',
    '        두 배지가 함께 붙을 수 있다. 예: "조건 확인! + 생각 더하기"가 함께',
    '        보이면 둘 중 하나를 버리지 말고 label="조건 확인+생각 더하기"로 적어라.',
    ...
    '=== 출력 스키마 ===',
    '{',
    '  "section": "type_problem" | "unit_review" | "unknown",',
    '  "page_kind": "problem_page" | "concept_page" | "mixed" | "unknown",',
    '  "page_layout": "two_column" | "one_column" | "unknown",',
    '  "items": [',
    '    {',
    ...
    '      "content_group": {',
    '        "kind": "type" | "none",',
    '        "label": "<예: \\"유형 01\\", 없으면 빈 문자열>",',
    '        "title": "<유형 배지 오른쪽의 유형명. 예: \\"지수법칙의 표현\\">",',
    '        "order": <유형 번호 정수 또는 null>',
    '      },',
    '      "column": 1 | 2 | null,',
    '      "bbox": [<ymin>, <xmin>, <ymax>, <xmax>],',
    '      "item_region": [<ymin>, <xmin>, <ymax>, <xmax>]',
    ...
    '}',
    '',
    'type_headers 에는 이 지면에 인쇄된 분홍색 "유형 NN" 머리말 줄(배지 + 유형명',
    '띠 전체)의 위치를 모두 적는다. 머리말은 문항이 아니지만 앞 문항 크롭의 아래',
    '경계를 정하는 데 쓰이므로 위쪽 y 좌표가 정확해야 한다. 없으면 빈 배열.',
    '',
    'section 은 이 지면에서 문항 수가 가장 많은 코너를 적는다(개념 체크는 소단원',
    '본문에 속하므로 type_problem 으로 센다). 문항이 없으면 "unknown".',
    '',
    '=== 탐지 규칙 ===',
    '[S1] 지면에 보이는 문항을 **하나도 빠뜨리지 마라.** 이 교재의 문항은 짧아서',
    '     한 지면에 20개가 넘게 들어가기도 한다. 번호가 01, 02, 03 … 으로 1씩',
    '     이어지는지 좌단 위에서부터 우단 아래까지 훑어 확인하고, 중간에 빠진',
    '     번호가 있으면 그 자리를 다시 봐라. 빠진 문항은 영영 저장되지 않는다.',
    ...
    '     머리말이다. items 에 넣지 말고, 그 아래 문항들의 content_group 에',
    '     kind="type", label="유형 01", title=유형명, order=1 처럼 채워라.',
    '     유형 번호는 대단원 안에서 계속 이어지므로 한 지면에 유형이 두세 개',
    '     나올 수 있다. 문항마다 **자기 바로 위 유형**을 적어야 한다.',
    '     - 유형 머리말은 유형이 바뀔 때 한 번만 인쇄된다. 좌단에서 시작한 유형은',
    '       우단으로 넘어가도, 다음 지면으로 넘어가도 새 머리말이 나오기 전까지',
    '       그대로 이어진다. 머리말이 안 보인다고 kind="none" 으로 두지 말고',
    '       **직전 유형을 그대로 반복해 적어라.** 우단 첫 문항에서 유형이 끊기는',
    '       것이 이 지면에서 가장 흔한 실수다.',
    '     - 개념 체크(category="concept_check")는 유형에 딸린 문항이 아니다.',
    '       항상 kind="none" 으로 두어라.',
    '[S4] 대괄호 범위 지문("[01-05] 다음을 거듭제곱으로 나타내어라.")은 아래',
    '     문항들이 공유하는 공통 지문이다. 이것도 items 에 한 줄로 담되',
    ...
    '     - 가운뎃점·하이픈·물결 어느 것으로 이어졌든 범위다("[08-13]", "[08~13]").',
    '       number 에는 대괄호를 빼고 "08~13" 으로 적어라.',
    '     - **좌단·우단 양쪽을 모두 훑어라.** 이 지면에서 가장 흔한 누락은',
    '       지면 맨 위, 유형 머리말 없이 시작하는 범위 지문이다. 앞 지면에서',
    '       이어진 세트라 머리말이 없을 뿐이고, 똑같이 담아야 한다.',
    '     - 범위 지문을 빠뜨린 채 그 아래 개별 문항만 담으면 그 문항들은 공통',
    '       지문 없이 저장되어 무슨 문제인지 알 수 없게 된다.',
    ...
    '     문항으로 감싸지 마라.',
    '[S7] 레이아웃이 2단이면 column 은 좌측단 = 1, 우측단 = 2. 1단이거나 판단',
    '     불가이면 null. items 는 좌단 위→아래, 그다음 우단 위→아래 순으로 정렬.',
    '[S8] bbox 는 **문항 번호 숫자만** 감싸는 최소 박스다. 이 좌표가 크롭 위치의',
    '     기준이 되므로 지면에서 가장 정확해야 하는 값이다. 번호는 문항의',
    '     **왼쪽 위 모서리**에 있고 본문 첫 줄이 그 오른쪽 같은 줄에서 시작한다.',
    '     좌표계: 이미지 좌상단 (0,0), 우하단 (1000,1000). [ymin, xmin, ymax, xmax].',
    '[S9] item_region 은 그 문항이 차지하는 지면 영역이다. 그대로 크롭 이미지로',
    '     잘려 저장된다.',
    '     - 위 경계: **번호와 같은 줄**(번호 bbox 의 위쪽). 본문 첫 줄이 번호와',
    '       같은 줄에서 시작하므로 번호 아래부터 잡으면 첫 줄이 잘린다.',
    '     - 아래 경계: **같은 단의 다음 문항 번호 바로 위**. 문항 사이 빈 공간은',
    '       답을 적는 자리라 그 문항의 일부다. 지면 마지막 문항이면 단 아래쪽',
    '       여백 끝(쪽번호·"< 정답과 해설 p. 22 >" 안내 위)까지.',
    '     - 좌우 경계: **그 단의 폭 전체**. 둘째 줄부터는 번호 아래까지 글이',
    '       내려오므로 번호 오른쪽부터 잡으면 왼쪽이 잘린다.',
    '     - 유형 머리말, 개념 박스, 이웃 단은 넣지 않는다. 문항 아래에 다음 유형',
    '       머리말이 오면 아래 경계는 그 머리말 **위**까지다.',
    '     - 두 문항의 item_region 은 서로 겹치지 않아야 한다.',
    '',
    '=== 절대 금지 ===',
    '[N1] stem, choices, answer 같은 다른 필드를 만들지 마라. 번호 위치 탐지 전용이다.',
    '[N2] 없는 문항을 추측해서 만들지 마라. 보이는 것만 담는다.',
    '',
    '지금 첨부된 이미지를 분석해 위 스키마로만 출력하라.',
  ];
  ...
  return [
    ...
    ...(isSkillTest
      ...
      ? [
          ...
          '한 번호에 두 배지가 함께 붙을 수 있습니다. "조건 확인! + 생각 더하기"면',
          'label을 정확히 "조건 확인+생각 더하기"로 적으세요.',
        ]
      : ['label은 항상 빈 문자열입니다.']),
    ...(!unitReview
      ? [
          '또한 문항 사이에 보이는 파란 테두리의 유형 설명 배지/도움말 제목 줄을',
          'breaks에 담으세요. 예: "원의 중심과 직선 사이의 거리 이용". 이 줄은',
          '앞 문항 크롭의 끝 경계이며 문항은 아닙니다.',
        ]
      : []),
    '',
    'JSON만 출력:',
    '{ "items": [',
    '  { "number":"02", "label":"", "bbox":[ymin,xmin,ymax,xmax] }',
    '], "breaks": [',
    '  { "title":"원의 중심과 직선 사이의 거리 이용", "bbox":[ymin,xmin,ymax,xmax] }',
    '] }',
    'bbox는 번호 숫자만 감싸는 0..1000 [ymin,xmin,ymax,xmax] 좌표입니다.',
  ].join('\n');
}
...
function buildGaeyuDetectPrompt({ displayPage, rawPage }) {
  ...
  const lines = [
    ...
    '      아래로 다음이 순서대로 이어진다.',
    '      · 개념 카드: 상자 제목 앞의 굵은 숫자("3 소인수분해")는 개념 번호이지',
    '        문항 번호가 아니다. 절대 items 에 넣지 마라.',
    '        개념 카드는 보통 **노란(크림색) 배경**이지만, 드물게 **파란 테두리에',
    '        "교과서 +α" 배지**가 붙은 카드로도 나온다("교과서에는 없지만 시험에',
    '        자주 출제되는 내용입니다"). 배경색·테두리색이 무엇이든 개념 카드이며,',
    '        제목 앞 숫자가 빨간 네모 안에 있어도 개념 번호다. items 에 넣지 마라.',
    '      · "개념 확인" 전용 원형 배지: 원 안에 초록색 "개념"과 검정색 "확인"이',
    '        위아래 **2줄**로 들어 있다. 이 원형 배지가 붙은 문항만 개념확인이다.',
    '        문항 위치는 판정 근거가 아니다. 보통 개념 박스 아래지만 다른 위치에도',
    '        올 수 있다. **인쇄된 번호가 없다.** → category="concept_check",',
    '        badge_text="개념 확인", badge_style="concept_check_round_two_line",',
    '        number="" 로 두어라.',
    ...
    '      · **두 배지를 섞지 마라.** 판별은 배지 글자 그대로 읽으면 끝난다.',
    '        "개념 확인"이라고 적혀 있으면 concept_check, "필수 문제"라고 적혀',
    '        있으면 essential_problem 이다. 개념 카드 바로 아래에 있다는 이유로',
    '        필수 문제를 개념확인으로 바꾸지 마라. 개념+유형은 개념 설명 밑에',
    '        필수 문제가 바로 붙는 것이 기본 구성이다.',
    '        번호로도 갈린다. **번호가 인쇄돼 있으면 개념확인이 아니다.**',
    '        원 안에 숫자가 있거나 "1-1" 같은 번호가 보이면 essential_problem 이고,',
    '        번호가 아예 없으면 concept_check 다.',
    '      · 필수 문제 아래로 "1-1", "1-2", "2-1" 처럼 **하이픈이 붙은 번호(N-M)**',
    '        가 이어진다. 같은 필수 문제에 딸린 따름 문제다.',
    '        → category="essential_problem", number="1-1" 처럼 하이픈 포함 원문 그대로.',
    '        **이 양식은 개념이 절대 쓰지 않는다. 보이면 무조건 문항이다.**',
    '        "필수 문제" 배지는 대표 번호(1, 2, 3…)에만 붙고 따름 번호에는 없다.',
    '        배지가 없어도, 지면 **맨 위**에만 있어도(앞 페이지에서 이어진 경우),',
    '        개념 카드 옆·아래에 있어도 **빠뜨리지 마라.**',
    '  (2) 쏙쏙 개념 익히기 — 상단에 주황색 "STEP 1" 원 배지와 "쏙쏙 개념 익히기".',
    '      굵은 숫자 1, 2, 3 … 이 세로로 나열된다.',
    '      → category="step_drill", label="".',
    '      쏙쏙은 여러 지면에 걸쳐 이어진다. 상단 배지는 첫 지면에만 있고, 이어지는',
    '      지면은 배지 없이 번호만 계속된다(1~5쪽에 이어 6~10쪽). 배지가 안 보여도',
    '      번호가 앞 지면에서 이어지면 그대로 step_drill 이다.',
    ...
    '      단, 문항 번호 **옆(왼쪽 여백)** 에 붙는 작은 초록 원형 "한번더 +1" 배지는',
    '      이것과 다르다. 바로 위 문항과 세로선으로 이어진 쌍둥이 문항 표시일 뿐이라',
    '      코너가 바뀌지 않는다. 그 문항도 주변 문항과 같은 category(대개 step_drill)',
    '      로 두고 번호도 이어진 그대로 쓴다.',
    '      판별 기준 두 가지를 같이 봐라. ① 배지가 지면 맨 위 머리말 자리에 소단원명과',
    '      함께 있는가, ② 그 지면 첫 문항 번호가 1인가. 번호가 6, 7 처럼 이어지면',
    '      그건 앞 지면에서 계속되는 쏙쏙이지 한 번 더 연습이 아니다.',
    ...
    '      "badge_text": "<이 문항에 붙은 코너 배지의 인쇄 문구 원문. 예: \\"개념 확인\\" 또는 \\"필수 문제\\". 배지가 없으면 \\"\\">",',
    '      "badge_style": "<개념확인 전용 2줄 원형 배지면 \\"concept_check_round_two_line\\", 그 외에는 \\"\\">",',
    ...
    '     "STEP 1 쏙쏙 개념 익히기" → step_drill, 머리말의 "한번 더 연습" → extra_practice',
    '     (문항 옆 작은 "한번더 +1" 배지는 코너 배지가 아니다. D0-Cat 근거로 쓰지 마라),',
    '     "STEP 2 탄탄 단원 다지기" → unit_drill, "STEP 3 쓱쓱 서술형 완성하기" → descriptive.',
    '     상단 배지가 없는 본문 지면에서만 "개념 확인"/"필수 문제" 배지로 판단한다.',
    '[D0-Step] STEP 코너는 한 중단원 안에서 STEP1 → STEP2 → STEP3 순서로만 간다.',
    '     STEP2 나 STEP3 가 한 번 시작되면 그 중단원이 끝날 때까지 STEP1(쏙쏙)로',
    '     되돌아가지 않는다. 탄탄은 세 쪽까지 이어지고 이어지는 쪽에는 "STEP 2"',
    '     배지가 다시 인쇄되지 않으니, 배지가 안 보이면 직전 지면의 코너를 그대로',
    '     이어받아라. 배지 없는 이어지는 지면을 step_drill 로 되돌리면 틀린다.',
    '[D0-Label] label 은 그 코너에 **실제로 인쇄된 표시**만 옮겨 적는다.',
    '     난이도 "상/중/하"는 번호 위에 작은 원 세 개가 찍히는 탄탄(unit_drill)',
    '     에만 있다. 쏙쏙·필수 문제·개념확인·한 번 더 연습에는 난이도가 아예',
    '     인쇄되지 않으니 label="" 로 두어라. 원 세 개가 안 보이는데 "중" 을',
    '     써 넣는 것은 지어내는 것이다. 갈래 "예제/유제/연습"은 쓱쓱 전용이다.',
    '[D0-Follow] "1-1", "1-2", "2-1", "7-2" 처럼 **숫자-숫자(하이픈) 양식은',
    '     무조건 따름 문제**다. 개념 번호·개념 카드·쪽번호·소주제 라벨이 아니다.',
    '     개념에는 하이픈 번호가 없다. 이 번호가 보이면 배지·위치·배경색과',
    '     무관하게 items 에 넣고 category="essential_problem" 으로 둔다.',
    '     지면 맨 위·왼쪽 여백·앞 페이지에서 이어진 경우도 예외 없다.',
    '[D0-Concept] 문항이 아닌 것들 — 절대 items 에 넣지 마라:',
    '     - 개념 카드 제목 앞의 굵은 숫자(개념 번호). 노란 카드든 파란',
    '       "교과서 +α" 카드든 동일하다. 개념 번호는 "3", "4" 처럼 **하이픈 없는**',
    '       한 덩어리 숫자다. "2-1" 같은 N-M 은 여기 해당하지 않는다(→[D0-Follow]).',
    '     - 지면 상단의 **노란(주황) 가로줄 아래 크림색 배경 카드**. "2 거리, 속력,',
    '       시간에 대한 문제", "4 x절편과 y절편을 이용하여 일차함수의 그래프 그리기"',
    '       처럼 굵은 숫자 + 명사구 제목이 **같은 줄에 나란히** 있고, 안에는 정의·공식과',
    '       "주의"·"참고" 캡슐이 들어 있다. 이 숫자는 개념 번호다. 왼쪽에 "필수 문제"',
    '       라벨이 없고 앞 지면의 쏙쏙·탄탄을 이어받는 중이어도 문항으로 올리지 마라.',
    '       단 제목이 아니라 본문 왼쪽의 "2-1" 번호 배지는 따름 문제다.',
    '     - 왼쪽 여백의 회색 도움말/공식 정리 상자와 그 안의 화살표 항목.',
    '     - 오른쪽 여백의 "용어" 상자, 참고 말풍선.',
    '     - 파란색/청록색 캡슐에 "참고"라고 적힌 설명 상자. 개념 박스 바로',
    '       아래에 있어도 문항이 아니며 concept_check가 아니다.',
    '     - "정답과 해설 17쪽" 같은 안내 문구와 쪽번호.',
    '     - 개념 Review·마인드맵의 빈칸 번호.',
    '[D0-Essential] 대표 필수 문제(하이픈 **없는** 번호 1, 2, 3…)만 **"필수 문제"',
    '     보라색 배지가 그 번호와 같은 줄에 실제로 보일 때** essential_problem 으로',
    '     올린다. 배지가 없으면 한 자리/두 자리 숫자를 필수 문제로 올리지 마라.',
    '     특히 개념 카드의 개념 번호를 필수 문제로 올리는 실수가 잦다.',
    '     **예외: "1-1", "2-1" 같은 N-M 하이픈 번호는 배지 없이도 무조건',
    '     essential_problem** 이다([D0-Follow]). 배지 게이트를 따름 문제에',
    '     적용하지 마라.',
    '     대표 번호를 볼 때마다 이 순서로 판정하라.',
    '     1) 번호 **왼쪽**에 보라색 "필수 문제" 라벨이 붙어 있는가? 이 라벨 바로',
    '        아래에는 유형명("x절편과 y절편을 이용하여 일차함수의 그래프 그리기")이',
    '        두세 줄로 인쇄된다. 라벨이 보이면 essential_problem 이다.',
    '     2) 라벨이 없고 번호 **오른쪽 같은 줄에** 제목이 바로 이어지는가?',
    '        그것은 개념 번호다. items 에 절대 넣지 마라.',
    '     보조 근거: 개념 카드 안은 정의·공식·예시 설명이라 묻는 문장이 없고,',
    '     필수 문제는 "~을 구하시오", "~인 것은?" 처럼 묻는 문장이 이어진다.',
    '     개념 카드 제목을 content_group.title(유형명)로 쓰지 마라.',
    '[D0-Order] 한 지면의 필수 문제 번호는 **위에서 아래로 커진다.** 개념 번호는',
    '     중단원 전체를 통틀어 매기므로 그 지면 필수 문제 번호보다 앞서 가 있다.',
    '     그래서 지면 위쪽 숫자가 그 아래 필수 문제 번호보다 **크면** 그것은',
    '     개념 번호다. 예: 크림색 카드의 "4 피타고라스 정리의 증명 (3)" 아래에',
    '     "필수 문제 3", "3-1" 이 오는 지면 — 4 는 개념 번호이므로 items 에',
    '     넣지 마라. 넣으면 다음 지면의 진짜 필수 문제 4번과 번호가 겹쳐',
    '     그 문항이 사라진다.',
    '[D0-Mix] 쏙쏙 개념 익히기(STEP1)는 자기 전용 지면에 1번부터 연속으로',
    '     인쇄된다. **필수 문제·따름 문제가 있는 지면에는 쏙쏙이 섞이지 않는다.**',
    '     필수 문제가 보이는 지면에서 앞 지면 쏙쏙 번호를 이어받아 step_drill',
    '     하나를 얹는 것은 개념 번호 오인식이다. 그런 숫자는 개념 번호다.',
    '[D0-Check] concept_check 는 **초록색 "개념"+검정색 "확인"이 원 안에',
    '     2줄로 표시된 전용 배지가 실제로 보이고**, **그 문항에 인쇄된 번호가',
    '     없을 때만** 쓴다. 두 조건을 모두 만족해야 한다.',
    '     번호가 보이는 문항에 concept_check 를 붙이면 그 번호가 버려지고 지면의',
    '     "개념확인<쪽번호>" 로 덮여 문항 하나가 통째로 사라진다. 개념 설명 바로',
    '     아래에 있다는 위치만 보고 개념확인으로 넘겨짚지 마라. "참고" 캡슐은',
    '     모양과 글자가 모두 다르며, 주변에 질문·답안 칸이 있어 보여도 제외한다.',
    '     category="concept_check" 를 골랐으면 badge_text="개념 확인" 과',
    '     badge_style="concept_check_round_two_line" 을 **반드시 함께** 채워라.',
    '     이 두 칸이 그 배지를 실제로 봤다는 근거다. 지면에 개념확인이 있는데도',
    '     items 에서 빠뜨리면 그 문항은 통째로 유실된다.',
    '[D0-Badge] category를 고르기 전에 각 문항에 실제로 붙은 코너 배지 글자를',
    '     badge_text에 **보이는 그대로 베껴 써라.** "개념 확인"과 "필수 문제"를',
    '     추측해서 바꾸지 마라. 필수 문제 배지가 보이면 badge_text="필수 문제",',
    '     category="essential_problem"이고, 개념 확인 배지가 보이면',
    '     badge_text="개념 확인", badge_style="concept_check_round_two_line",',
    '     category="concept_check"다. "참고"면 badge_text="참고",',
    '     badge_style=""로 쓰고 items에는 넣지 마라.',
    '[D1] 본문 안의 "(1), (2)" 같은 소문항 레이블, "①~⑤" 선택지 기호,',
    '     "풀이 과정", "1단계", "답" 같은 섹션 헤더는 문항 번호가 아니다.',
    '     단, "1-1", "1-2", "2-1" 처럼 하이픈으로 이어진 번호는 따름 문제이므로',
    '     반드시 수집한다([D0-Follow]). 개념으로 착각해 빼면 안 된다.',
    '     개념+유형에서는 대표 번호 아래 (1), (2)가 여러 개 있어도 독립 문항이 아니다.',
    '     대표 번호 item 하나만 수집하고 is_set_header=false, set_range=null 로 둔다.',
    '     단, 실제 문제 번호 배지에 "1-1", "1-2"로 인쇄된 따름 문제는 내부의',
    '     (1), (2)와 전혀 다르다. 같은 유형 복습 문항이므로 반드시 각각 별도 수집한다.',
    '[D2] 페이지 번호(쪽 번호)는 문항 번호가 아니다. 넣지 마라.',
    '[D3] 레이아웃이 2단이면 column 은 좌측단 = 1, 우측단 = 2. 1단이거나 판단 불가이면 null.',
    '[D4] bbox 는 "문항번호 숫자 및 바로 옆 배지까지만" 감싸는 최소 박스.',
    '     개념확인처럼 번호가 없으면 "개념 확인" 배지 자체를 bbox 로 잡는다.',
    '     좌표계: 이미지 좌상단 (0,0), 우하단 (1000,1000). 순서는 [ymin, xmin, ymax, xmax].',
    '[D4-필수] 대표 필수 문제의 보라색 원 번호(1, 2, 3...)는 문제 본문의',
    '     **왼쪽 바깥에서 첫 문장과 같은 높이**에 있다. bbox 는 그 원 번호를 잡아야 한다.',
    '     본문 위 빈 공간이나 유형명 아래 빈 칸을 bbox 로 잡지 마라.',
    '     "1-1", "2-1" 같은 따름 문제 번호는 본문 왼쪽 위에 인쇄된 번호 자체를 잡는다.',
    '[D4-쓱쓱] 쓱쓱 서술형(STEP 3)의 번호 배지 위치를 특히 주의하라.',
    '     - 예제·유제: "예제\\n1", "유제\\n1" 처럼 글자와 숫자가 **2줄로 쌓인 둥근 배지**가',
    '       문제 상자의 **왼쪽 바깥**에 붙어 있다. 예제는 보라색, 유제는 파란색이다.',
    '       bbox 는 이 배지 도형 자체를 감싼다. 배지의 세로 위치는 문제 상자 첫 줄과',
    '       나란하다 — 문제 상자보다 **위쪽 빈 공간을 잡으면 틀린 것이다.**',
    '     - 연습해 보자: 번호가 문제 상자 **왼쪽 위 바깥**의 네모 안 굵은 숫자다.',
    '       그 숫자 네모만 bbox 로 잡는다.',
    '     - "STEP 3" 원 배지, "쓱쓱 서술형 완성하기" 제목, "따라 해보자" 리본,',
    '       "연습해 보자" 리본은 코너 헤더이지 문항 번호가 아니다. bbox 로 잡지 마라.',
    '[D5] item_region 은 이 문항의 본문 영역만 타이트하게 감싸는 박스다.',
    '     그대로 크롭 이미지로 잘라 저장되므로 본문이 아닌 것은 어떤 것도 들어가면 안 된다.',
    '     - 문항 번호와 배지는 bbox 에만 담고 item_region 에서 제외한다.',
    '     - **포함**: 지문, 보기 상자, 조건 상자, 표, 그림, 선택지(①~⑤), 소문항 "(1)/(2)".',
    '     - **제외**: 왼쪽/오른쪽 여백의 도움말·용어 상자, 이웃 문항, 코너 헤더.',
    '     - **쓱쓱 서술형 3갈래 공통(예제·유제·연습해 보자)**: 물음이 적힌 문제 상자까지만',
    '       감싼다. 그 아래 "풀이 과정" 라벨과 "1단계/2단계/3단계", 답을 적는 빈 칸,',
    '       맨 아래 "답" 줄은 **전부 제외**한다. 아래 경계는 "풀이 과정" 글자가',
    '       시작되기 직전이다. 예제처럼 풀이가 이미 인쇄돼 있어도, 유제·연습해 보자처럼',
    '       칸이 비어 있어도 규칙은 같다. 빈 여백을 같이 크롭하면 실패다.',
    '     - 연습해 보자는 문제 상자와 풀이 칸이 하나의 큰 테두리 안에 같이 들어 있어서',
    '       테두리 전체를 잡기 쉽다. 테두리가 아니라 **물음 문장과 딸린 표·그림이',
    '       끝나는 지점**을 아래 경계로 삼아라. 소문항 "(1) (2)" 물음은 포함하되,',
    '       그 답을 적는 아래 빈 칸은 포함하지 않는다.',
    '     - 각 변마다 0..1000 스케일 기준 6~12 정도의 아주 작은 여백만 둔다.',
    '     - 두 문항의 item_region 은 원칙적으로 서로 겹치지 않아야 한다.',
    '[D6] content_group 규칙:',
