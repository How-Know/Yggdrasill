...
export function buildExtractAnswersPrompt({
  ...
  return [
    ...
    '   - 정답 일부에만 그림이 포함되어도 반드시 kind="image" 로 둔다. 텍스트가 같이 보여도 subjective 로 낮추지 마라.',
    '     예: "(1)[image] (2) 34", "41 [image] 5개", "42 [image] 이므로 12".',
    '   - answer_text 에는 텍스트 부분을 보존하되 그림 자리에는 "[image]" 를 넣는다.',
    '   - answer_latex_2d="" 로 둔다.',
    '   - bbox 는 [image] 에 해당하는 그림/표/격자 정답 전체를 타이트하게 감싼다. 텍스트와 그림이 한 덩어리이면 둘 다 포함해도 된다.',
    '',
    '[R3] 문항번호 매칭',
    ...
    '   - 답지의 "C 만점 도전하기", "만점", "서술형", "도전" 구간 정답도 반드시 포함한다.',
