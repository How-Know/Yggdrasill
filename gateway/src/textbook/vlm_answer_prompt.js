...
export function buildExtractAnswersPrompt({
  ...
  const expectedBlock = expected.length
    ? [
        ...
        '각 번호에 대한 정답을 빠짐없이 찾아서 items 에 담아라. ',
        '찾지 못한 번호는 answer_text 를 "" (빈 문자열) 로 두되, 그래도 item 자체는 포함하라.',
        '',
  ...
  return [
    ...
    '      "answer_text": "<객관식이면 ①~⑤ 중 하나. 주관식이면 1D LaTeX 원문. 그림 정답이면 \\"[image]\\". 찾지 못하면 빈 문자열>",',
    '      "answer_latex_2d": "<주관식일 때 2D 렌더에 쓸 LaTeX. 객관식/그림이면 빈 문자열>",',
    ...
    '   - 수식 그대로를 LaTeX 로 변환해 answer_text 에 1D 형태로 담아라.',
    '     예: 분수 "3/4" → "\\\\dfrac{3}{4}", 루트 "√2" → "\\\\sqrt{2}", 지수 "a^2" → "a^{2}".',
    '   - 텍스트 답 ("해당없음", "풀이 참조" 등) 은 그대로 answer_text 에 담되 kind="subjective" 로 분류.',
    '   - answer_latex_2d 에는 편집기에서 그대로 써도 되는 LaTeX 를 넣어라. 단순한 경우는 answer_text 와 동일해도 좋다.',
    '',
    '[R2-Image] 그림 정답 (kind="image")',
    '   - 정답이 텍스트/수식이 아니라 도형, 그래프, 그림, 표기 이미지 자체로 제시되면 kind="image" 로 둔다.',
    '   - answer_text="[image]", answer_latex_2d="" 로 둔다.',
    '   - bbox 는 해당 그림 정답 전체를 타이트하게 감싼다. 나중에 이 bbox 그대로 크롭 저장한다.',
    '',
    '[R3] 문항번호 매칭',
    ...
    '',
    '[R4] 인식 불가',
    '   - 이미지가 번져 읽을 수 없거나, 해당 번호의 정답이 이 페이지에 없다고 판단되면 answer_text="" 로 둬라.',
