...
export function buildSolutionBlockIndexPrompt({ rawPage }) {
  return [
    ...
    '   - 쪽수는 **"p." 가 붙은 배지에서만** 가져와라. 머리글 이름에 딸린',
    '     대괄호 숫자("단원 마무리 평가 [16~22]")는 이 평가가 다루는 **소단원',
    '     번호 범위**지 쪽수가 아니다. 이 머리글의 쪽 배지는 오른쪽에 따로 붙은',
    '     작은 파란 상자 "▶문제편 p.55~59" 다 → page_start=55, page_end=59.',
    '     대괄호 숫자를 쪽수로 적으면 이 블록의 풀이가 통째로 버려진다.',
...
function assignColumns(entries) {
  ...
  for (const entry of entries) {
    const xmin = entry.region[1];
    const xmax = entry.region[3];
    if (xmin < 500 && xmax > 500) {
      // 소단원 머리는 한 단 폭을 거의 다 채우는데, 왼쪽 끝이 중앙선에 한두
      // 픽셀 걸치는 일이 있다(3-1 해설 p81 머리 x=499~896). 왼쪽 끝만 보면
      // 그 머리가 왼쪽 단 한가운데로 끼어들어, 뒤따르는 번호가 통째로 다음
      // 소단원 것이 된다. 중앙선을 걸친 것은 넓게 걸친 쪽에 붙이고, 반반이면
      // 지면 전체를 덮는 머리로 보아 왼쪽(먼저 읽는 쪽)에 둔다.
      entry.column = 500 - xmin >= xmax - 500 ? 0 : 1;
    } else {
      entry.column = xmin < 500 ? 0 : 1;
    }
  }
  return bounds;
}
