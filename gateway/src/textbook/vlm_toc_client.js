...
export function buildParseTocPrompt({ pageCount, series = '' }) {
  ...
  const gaeyuLines =
    seriesKey === 'gaeyu'
      ? [
          '',
          '=== 개념+유형 교재 목차 규칙 (매우 중요) ===',
          '이 교재(개념+유형)의 목차는 3단계다:',
          '  - 대단원: 왼쪽 여백에 "I 수와 연산" 처럼 로마숫자 배지와 함께 표기 → big_units[].name.',
          '    로마숫자는 버리고 이름만 담아라 (예: "수와 연산").',
          '  - 중단원: "1 소인수분해" 처럼 큰 숫자와 제목으로 표기 → mid_units[].name. 이름만 담아라.',
          '    중단원 줄 자체에는 페이지가 인쇄돼 있지 않다 — mid_units[].page 는 null 로 둔다.',
          '  - 소단원: "O1 소인수분해 ......... 8" 처럼 두 자리 번호 + 이름 + 점선 + 페이지 → sub_units[].',
          '    번호는 버리고 이름만 name 에, 줄 오른쪽 페이지 숫자를 page 에 담아라.',
          '각 중단원의 번호 붙은 소단원들 뒤에는 "•" 글머리표로 시작하는 마무리 줄이 두 개 이어진다:',
          '  "• 단원 다지기 / 서술형 완성하기 ......... 21"',
          '  "• 개념 리뷰 / 마인드맵 ......... 26"',
          '이 두 줄도 sub_units 에 인쇄된 순서 그대로 넣되, 이름은 인쇄된 문구 그대로 쓰고',
          '{ "is_exercise": true } 로 표시하라. 페이지 숫자도 각각 반드시 채워라.',
          '번호 붙은 일반 소단원은 is_exercise: false 다.',
          '**매우 중요**: 모든 sub_units 항목의 page 를 점선 오른쪽에 인쇄된 숫자로 빠짐없이 채워라.',
          '페이지 숫자를 절대 null 로 비우지 마라 (정말 안 보이는 경우에만 null).',
          '"개념확인", "필수 문제", "따름 문제", "쏙쏙 개념 익히기", "탄탄 단원 다지기",',
          '"쓱쓱 서술형 완성하기", "한번 더 연습", "STEP" 같은 문제 카테고리 라벨은 단원이 아니다',
          '— 목차에 보이더라도 어느 레벨에도 절대 넣지 마라.',
          '목차 페이지 구석의 다른 학년 교재 안내표(예: "개념플러스유형 1-2" 표)는 목차가 아니다. 무시하라.',
        ]
      : [];
  const suryeokLines =
    seriesKey === 'suryeok'
      ? [
          '',
          '=== 수력충전 교재 목차 규칙 (매우 중요) ===',
          '이 교재(수력충전)의 목차는 3단계다:',
          '  - 대단원: "I 지수함수와 로그함수" 처럼 색 배지 안의 로마숫자 + 큰 제목',
          '    → big_units[].name. 로마숫자는 버리고 이름만 담아라.',
          '  - 중단원: "1. 지수" 처럼 숫자 + 제목 → mid_units[].name. 이름만 담아라.',
          '    중단원 줄에는 페이지가 인쇄돼 있지 않다 — mid_units[].page 는 null 로 둔다.',
          '  - 소단원: "01 거듭제곱과 지수법칙 ......... 10" 처럼 두 자리 번호 + 이름',
          '    + 점선 + 페이지 → sub_units[]. 번호는 버리고 이름만 name 에,',
          '    줄 오른쪽 페이지 숫자를 page 에 담아라.',
          '소단원은 한 중단원에 10~15개씩 아주 많다. **한 줄도 빠뜨리지 마라.**',
          '소단원 번호는 대단원마다 01 로 돌아가고 그 안에서는 중단원을 넘어 계속',
          '이어진다. 예: 대단원 I 이 01~49 까지 가고 대단원 II 가 다시 01 로 시작한다.',
          '번호가 이어지는지 확인하면 빠뜨린 줄을 스스로 찾을 수 있다.',
          '각 중단원의 소단원 목록 끝에는 "• 단원 마무리 평가 ......... 29" 한 줄이',
          '붙는다. 이 줄도 sub_units 에 그 자리 그대로 넣고 { "is_exercise": true }',
          '로 표시하며 페이지도 반드시 채워라.',
          '소단원 이름이 길어 두 줄로 접혀 인쇄된 경우(둘째 줄에 페이지 숫자가 있는',
          '경우) 두 줄을 한 이름으로 이어 붙이고 페이지는 한 번만 담아라.',
          '일부 소단원 줄은 노란 형광 배경에 "★" 가 붙어 있다(교육과정 외 추가 개념).',
          '표시만 다를 뿐 똑같은 소단원이니 ★ 는 버리고 이름·페이지를 그대로 담아라.',
          '목차 아래쪽의 "★ 교육과정 외 추가 개념으로 …" 안내 문구는 단원이 아니다.',
          '목차 맨 끝의 별 배지 "학교 시험 대비 실력 향상 테스트" 묶음은 부속물이',
          '아니라 단원이다. big_units 의 **맨 뒤에** 대단원을 하나 더 만들어',
          '{ "name": "학교 시험 대비 실력 향상 테스트" } 로 담아라.',
          '그 아래 "Ⅰ단원 실력 향상 테스트 ......... 200" 같은 줄 하나하나를',
          'mid_units 로 담되, 이름은 로마숫자를 **빼지 말고** 인쇄된 그대로',
          '("Ⅰ단원 실력 향상 테스트") 넣어 세 줄이 서로 구분되게 하고,',
          '오른쪽 페이지 숫자는 mid_units[].page 에 담아라.',
          '각 중단원의 sub_units 에는',
          '{ "name": "실력 향상 테스트", "page": <그 중단원과 같은 페이지>,',
          '  "is_exercise": true } 한 줄만 담는다.',
          '이 묶음이 없는 교재도 있으니 목차에 보이지 않으면 만들지 마라.',
          '"〈개념 찾아보기〉" 는 트리에 넣지 말고, 그 오른쪽 페이지 숫자를',
          'appendix_boundary_page 에 담아라.',
          '"유형", "개념 체크", "계산 조심", "생각 더하기", "조건 확인", "DAY" 같은',
  ...
  return [
    ...
    '          "sub_units": [',
    '            {',
    '              "name": "<소단원 이름. 앞에 붙는 번호는 빼고 이름만. 위 시리즈 규칙이 있으면 그 규칙을 따른다>",',
    '              "page": <목차에 인쇄된 시작 페이지 숫자 또는 null>,',
    '              "is_exercise": <bool — 중단원 끝의 마무리 문제 묶음(연습문제 / 단원 다지기 등)이면 true, 일반 소단원이면 false>',
    '            }',
    '          ]',
    '        }',
    '      ]',
    ...
    '     단, 위 시리즈 규칙이 트리에 넣으라고 명시한 항목은 예외다.',
