...
export function buildAnswerLayoutPrompt({ rawPage }) {
  return [
    ...
    '     정답 속 숫자, 원문자 ①~⑤, 지면 쪽 번호.',
    '[A8] 초록색 번호가 "05~09" 처럼 **범위로** 인쇄된 묶음이 있다. 그 아래에는',
    '     좌표평면·표·격자 그림 한 장만 있고 문항별 답이 따로 없다.',
    '     - problem_number 에 "05~09" 처럼 **범위 그대로** 적어라.',
    '       "05" 로 줄이면 나머지 문항의 정답이 사라진다.',
    '     - answer_kind="image", answer_text="[image]" 로 두고',
    '       bbox 는 그 그림 전체를 감싼다.',
    '[A9] 한 정답이 왼쪽 단 맨 아래에서 시작해 오른쪽 단 맨 위로 이어질 수 있다.',
    '     answer_text 에는 두 조각을 이어 한 정답으로 담되, bbox 는 **초록색',
    '     번호가 있는 조각만** 감싸라. 두 단을 함께 덮는 상자를 주면 그 정답이',
    '     지면 맨 위에서 시작한 것으로 읽혀 소단원이 어긋난다.',
    '',
    '=== 출력 ===',
    '{',
