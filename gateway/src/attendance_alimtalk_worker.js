...
function normalizeDateColumn(value) {
...
async function isAcademyNotificationPaused(academyId, eventDateKey) {
  ...
  const { data, error } = await supa
    ...
    .limit(1);
  if (error) {
    console.warn('[alimtalk-worker] pause-date check failed:', error.message);
    return false;
  }
...
async function isStudentPausedOn(academyId, studentId, eventDateKey) {
  ...
  if (error) {
    console.warn('[alimtalk-worker] student-pause check failed:', error.message);
    return false;
  }
  return data === true;
}
...
async function processBatch() {
  ...
  for (const row of queueRows) {
    ...
    try {
      ...
      const eventDateKey =
        normalizeDateColumn(attendance.date) ||
        getKstDateKey(attendance.class_date_time || attendance.arrival_time || attendance.departure_time);
      if (eventDateKey && eventDateKey < todayKstDateKey) {
        await setQueueStatus(row.id, { status: 'skipped', last_error: 'expired_event_date' });
        summary.skipped += 1;
        continue;
      }
      if (await isAcademyNotificationPaused(attendance.academy_id, eventDateKey)) {
        await setQueueStatus(row.id, { status: 'skipped', last_error: 'academy_notification_paused' });
        summary.skipped += 1;
        continue;
      }
      if (await isStudentPausedOn(attendance.academy_id, attendance.student_id, eventDateKey)) {
