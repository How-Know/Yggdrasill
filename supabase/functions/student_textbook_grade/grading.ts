...
// ---------------------------------------------------------------------------
// 규칙:
//   * 마커는 (1)부터 1씩 증가하거나 (가)부터 가나다순으로 증가하는 것만 인정
//     (다음 기대 마커만 탐색) → "(2)번 답이 (1)"처럼 내용에 등장하는 번호는
//     마커로 오인하지 않음. 첫 마커가 (1)이면 숫자, (가)면 한글 시퀀스.
//   * 마커는 문자열 시작 또는 공백 뒤에서만 인정
//   * 파트 내용이 비면 그 마커 후보는 내용으로 간주하고 건너뜀
//   * 파트 2개 미만, 마지막 내용 공백, 12파트 초과 → null (세트형 아님/파싱 포기)
export interface SetAnswerPart {
  key: string; // '(1)' | '(가)'
  text: string;
}

// 빈칸 채우기형 (가)(나)(다)… 마커 순서 (12개 = 숫자 상한과 동일).
...
export function splitSetAnswerParts(
  ...
  const keyOf = (n: number) =>
    korean ? `(${SET_PART_KOREAN_KEYS[n - 1]})` : `(${n})`;
  while (i < t.length) {
    const head = t.slice(i);
    let markerLen = 0;
    if (expected === 1) {
      const mn = numRe.exec(head);
      if (mn !== null && Number(mn[1]) === 1) {
        markerLen = mn[0].length;
        korean = false;
      } else {
        const mk = korRe.exec(head);
        if (mk !== null && mk[1] === SET_PART_KOREAN_KEYS[0]) {
      ...
      const mn = numRe.exec(head);
      if (mn !== null && Number(mn[1]) === expected) {
        markerLen = mn[0].length;
      }
    }
    if (markerLen > 0) {
