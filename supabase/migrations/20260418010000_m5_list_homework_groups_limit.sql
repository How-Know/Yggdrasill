-- Limit m5_list_homework_groups to at most 8 groups per student.
-- The M5 device UI can only display 6 groups (s_groups[6]) and parses at most 6.
-- Sending all 80+ groups causes a multi-KB MQTT payload that OOMs the ESP32.
-- Priority: running (phase 2) > submitted (3) > confirmed (4) > pending (1).
