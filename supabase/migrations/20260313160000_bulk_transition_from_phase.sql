-- Add p_from_phase parameter to only transition items of a specific phase.
-- Must drop old signature first (2 params) then create new one (3 params with default).

drop function if exists public.homework_group_bulk_transition(uuid, uuid);
