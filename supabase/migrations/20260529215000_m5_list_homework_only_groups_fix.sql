-- Fix: "column reference group_id is ambiguous" in m5_list_homework_only_groups.
-- The previous version used `g.id not in (select group_id from active_group_ids)`,
-- where bare `group_id` collided with the function's OUT parameter `group_id`.
-- Replace with an explicit, qualified NOT EXISTS.
