alter table public.homework_items
  add column if not exists test_origin_flow_id uuid
    references public.student_flows(id) on delete set null;

create index if not exists homework_items_academy_student_test_origin_flow_idx
  on public.homework_items (academy_id, student_id, test_origin_flow_id);
