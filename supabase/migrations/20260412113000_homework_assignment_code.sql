...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homework_items_assignment_code_format_chk'
  ) then
    alter table public.homework_items
      add constraint homework_items_assignment_code_format_chk
      check (
        assignment_code is null
        or assignment_code ~ '^[A-Z]{4}[0-9]{4}$'
      );
  end if;
end
$$;
...
create unique index if not exists homework_items_academy_assignment_code_uidx
...
begin
  ...
  if new.assignment_code is null and new.academy_id is not null then
    new.assignment_code := public.homework_issue_assignment_code(new.academy_id);
  end if;
  return new;
end;
$$;
...
before insert or update of assignment_code
...
begin
  ...
  loop
    loop
      v_code := public.homework_issue_assignment_code(v_row.academy_id);
      begin
...
begin
  ...
  loop
    ...
    values (
      ...
      case
        when nullif(
            trim(coalesce(elem->>'assignment_code', elem->>'assignmentCode', '')),
            ''
          ) is null
        then public.homework_issue_assignment_code(p_academy_id)
        when upper(trim(coalesce(elem->>'assignment_code', elem->>'assignmentCode')))
             ~ '^[A-Z]{4}[0-9]{4}$'
        then upper(trim(coalesce(elem->>'assignment_code', elem->>'assignmentCode')))
        else public.homework_issue_assignment_code(p_academy_id)
      end,
      nullif(elem->>'type', ''),
      nullif(elem->>'page', ''),
      (elem->>'count')::int,
