-- Keep group runtime aligned with child item phases after home-menu
-- group double-tap transitions. The client-facing RPC remains the same,
-- but the runtime row is reconciled inside the security-definer function.
...
begin
  ...
  if p_from_phase = 99 then
    ...
    perform public.m5_group_runtime_sync_from_children(p_academy_id, p_group_id, now());
    return v_rows;
  end if;
...
grant execute on function public.homework_group_bulk_transition(uuid, uuid, smallint) to anon, authenticated;
