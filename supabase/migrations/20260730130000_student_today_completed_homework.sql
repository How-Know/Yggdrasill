-- 학생앱: 오늘 완료(제출/완료)한 과제 그룹 목록
-- 학습앱 "오늘 완료"와 같이 submitted_at 또는 completed_at 이 오늘(Asia/Seoul)인 항목.
...
begin
  ...
  with done_items as (
    select
      ...
      h."type",
      h.content,
      coalesce(h.completed_at, h.submitted_at) as finished_at
    ...
    where h.academy_id = v_academy
      ...
      and (
        (h.submitted_at is not null
          and h.submitted_at >= v_day_start
          and h.submitted_at < v_day_end)
        or
