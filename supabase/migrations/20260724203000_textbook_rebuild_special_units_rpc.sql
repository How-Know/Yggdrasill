-- Repeatable rebuild of wonri special-lecture (E) small units for one book.
-- The initial normalization created these once; re-extraction of 특강 pages
-- must be able to (re)materialize the nodes so student/learning trees update.
...
begin
  ...
  -- Shift current children out of the way to avoid unique-order collisions.
