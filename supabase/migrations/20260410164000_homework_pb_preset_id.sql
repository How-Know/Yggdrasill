alter table public.homework_items
  add column if not exists pb_preset_id uuid
    references public.pb_export_presets(id) on delete set null;

create index if not exists homework_items_academy_student_pb_preset_idx
  on public.homework_items (academy_id, student_id, pb_preset_id);
