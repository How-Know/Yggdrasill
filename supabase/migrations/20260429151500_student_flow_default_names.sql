-- Rename legacy homework flows and seed the new default flow set.
-- 현행 -> 개념, 선행 -> 문제. Existing IDs are preserved so homework links stay valid.
...
update public.student_flows
set order_index = case trim(name)
  when '개념' then 0
  when '문제' then 1
  when '사고' then 2
  when '테스트' then 3
...
where trim(name) in ('개념', '문제', '사고', '테스트');
