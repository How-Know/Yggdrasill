-- 학생앱 계획 진행률 분자: 권장분 × 완료율
-- 완료율 = 교재/숙제 mastery 와 동일 (통과 문항 / 전체 문항, excluded 제외).
-- 문항 없는 과제(출력물 등)는 완료 시 1, 아니면 0.
...
create or replace function public.m5_items_earned_recommended_minutes(
  p_academy_id uuid,
  p_student_id uuid,
  p_item_ids uuid[]
)
...
set search_path = public
as $$
  -- 그룹 권장분(α 적용) × (권장가중 완료율).
  -- 완료율 가중 = Σ(item_rec × rate) / Σ(item_rec).
  with item_minutes as (
    select
      ...
      public.m5_item_completion_rate(
        p_academy_id,
        p_student_id,
        h.id
      ) as rate
  ...
  agg as (
    select
      coalesce(sum(minutes), 0)::numeric as raw_sum,
      coalesce(sum(minutes * rate), 0)::numeric as earned_raw,
  ...
  select case
    when p.raw_sum <= 0 or p.plan_minutes <= 0 then 0
    else least(
      p.plan_minutes,
      round(p.plan_minutes * (p.earned_raw / p.raw_sum))
    )::integer
  end
  from plan p;
$$;

revoke all on function public.m5_items_earned_recommended_minutes(uuid, uuid, uuid[])
  from public;
grant execute on function public.m5_items_earned_recommended_minutes(uuid, uuid, uuid[])
  to anon, authenticated;
