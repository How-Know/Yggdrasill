-- Record a structured grading outcome and restore the graded group to the
-- submitted shelf in one server transaction.

create or replace function public.homework_record_structured_grading(
  p_student_id uuid,
  p_group_id uuid,
...
begin
  if cardinality(coalesce(p_homework_item_ids, array[]::uuid[])) = 0 then
    raise exception 'HOMEWORK_STRUCTURED_GRADING_ITEMS_REQUIRED';
  end if;

  -- The existing outcome RPC performs membership, group/item and assignment
  -- validation and records checks with an idempotency key.
  v_result := public.homework_record_assignment_outcome(
    p_student_id,
    p_group_id,
    p_homework_item_ids,
    'graded',
  ...
  if v_academy_id is null then
    raise exception 'HOMEWORK_STRUCTURED_GRADING_GROUP_NOT_FOUND';
  end if;

  -- Keep the selected children together at the active shelf tail. Excluding
  -- the selected rows makes the order deterministic and avoids N client
  -- upserts with mapping resynchronization.
  select coalesce(max(h.order_index), -1)
  into v_base_order
  from public.homework_items h
  where h.academy_id = v_academy_id
    and h.student_id = p_student_id
    and h.status <> 1
    and not (h.id = any(p_homework_item_ids));

  for v_item_id, v_ordinality in
    select requested.item_id, requested.ordinality
    from unnest(p_homework_item_ids)
      with ordinality as requested(item_id, ordinality)
  loop
    update public.homework_items h
    set status = 0,
        order_index = v_base_order + v_ordinality::integer,
        completed_at = null,
        confirmed_at = null,
    ...
    if not found then
      raise exception 'HOMEWORK_STRUCTURED_GRADING_ITEM_NOT_FOUND:%', v_item_id;
    end if;

    -- Reuse the canonical phase-3 transition so accumulated time, phase
    -- events and M5 group runtime remain identical to the legacy path.
    perform public.homework_submit(
      v_item_id,
      v_academy_id,
      auth.uid()::text
    );
  ...
  return v_result || jsonb_build_object(
    'structured_transitioned_count', v_transitioned
  );
end;
$$;

revoke all on function public.homework_record_structured_grading(
  uuid, uuid, uuid[], integer, uuid, timestamptz
) from public;
grant execute on function public.homework_record_structured_grading(
  uuid, uuid, uuid[], integer, uuid, timestamptz
) to anon, authenticated;

-- Undo the most recent structured grading group as one transaction. This is
-- used when the teacher taps an already-created pending check.
...
begin
  ...
  select c.group_check_id
  into v_group_check_id
  ...
  where c.academy_id = v_academy_id
    ...
    and c.group_check_id is not null
  order by c.checked_at desc, c.id desc
  limit 1;

  if v_group_check_id is null then
  ...
  loop
    ...
    where c.academy_id = v_academy_id
      ...
      and c.group_check_id = v_group_check_id
      and c.outcome = 'graded'
    order by c.checked_at desc, c.id desc
    limit 1
    for update;
