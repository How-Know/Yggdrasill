-- 구조화 채점 반환 재시도는 assignment 보정보다 먼저 멱등 처리한다.
--
-- v2가 완료된 assignment를 먼저 assigned로 되살린 뒤 v1의 중복 응답을
-- 반환하면, 수업 중인 '오늘' 과제가 숙제로 노출된다. 그룹 advisory lock 안에서
-- request_id 중복을 먼저 확인해 재시도가 어떤 상태도 바꾸지 않게 한다.
...
begin
  ...
  -- 반드시 assignment 재활성화/생성보다 먼저 검사한다. 같은 request_id의
  -- 재전송은 최초 성공 결과를 그대로 인정하고 어떤 행도 수정하지 않는다.
  if exists (
    select 1
...
comment on function public.homework_commit_structured_grading_return_v2(jsonb) is
  '멱등 재시도를 상태 변경 없이 처리하고, 레거시 누락 assignment만 보정한 뒤 '
  '구조화 채점을 반환한다.';

-- 이미 같은 순서 문제로 되살아난 '오늘' 임시 assignment만 완료로 복구한다.
-- 실제 숙제는 destination=homework이거나 due가 있으므로 이 조건에 들어오지 않는다.
update public.homework_assignments a
set status = 'completed',
    updated_at = now(),
    version = coalesce(a.version, 1) + 1
where a.status in ('assigned', 'in_progress', 'carried_to_class')
  and a.due_date is null
  and a.due_at is null
  and exists (
    select 1
    from public.homework_session_plan_items spi
    where spi.academy_id = a.academy_id
      and spi.student_id = a.student_id
      and spi.homework_item_id = a.homework_item_id
      and spi.origin = 'planned_today'
      and spi.destination = 'in_class'
      and spi.rollover_policy = 'to_homework'
      and spi.resolution in ('pending', 'confirmed')
      and spi.assignment_id is null
  )
  and exists (
    select 1
    from public.homework_assignment_checks c
    where c.academy_id = a.academy_id
      and c.student_id = a.student_id
      and c.homework_item_id = a.homework_item_id
