-- 완료 개수: 목표 제시(snapshot_at) 이후 completed_at 기준.
-- 수리: 현재 오늘/다음 계획 + 스냅샷에 있던 과제 중 제시 이후 완료된 그룹.
...
begin
  ...
  if v_snapshot_at is not null
    ...
    -- 전체/완료 개수: 고정 그룹 스냅샷.
    -- 완료 = 모든 item에 completed_at이 있고, 그 완료 시각이 목표 제시 이후.
    select
        ...
        where jsonb_typeof(g.value -> 'item_ids') = 'array'
          ...
          and (
            ...
            where h.completed_at is not null
              and h.completed_at >= v_snapshot_at
...
-- 열린 세션 그룹 스냅샷 재작성:
-- 1) 오늘/다음 계획(완료 resolution 포함)
-- 2) 스냅샷에 있던 그룹 중 목표 제시 이후 전부 완료된 것
...
set homework_plan_snapshot_groups = coalesce((
  with snap_rows as (
    ...
    join public.homework_items h
      on h.id = s.item_id
     and h.academy_id = ar.academy_id
     and h.student_id = ar.student_id
    left join public.homework_group_items gi
      on gi.homework_item_id = s.item_id
  ...
  finished_after_plan as (
    ...
    having count(*) > 0
       and bool_and(
         ...
         and sr.completed_at >= ar.homework_plan_snapshot_at
       )
  ),
  combined as (
    select
    ...
    select sr.group_id, sr.item_id
    from snap_rows sr
    join finished_after_plan f on f.group_id = sr.group_id
  )
  select jsonb_agg(
    jsonb_build_object(
      'group_id', t.group_id,
  ...
  from (
    select
      c.group_id,
      jsonb_agg(distinct c.item_id order by c.item_id) as item_ids
    from combined c
    group by c.group_id
  ) t
), '[]'::jsonb)
