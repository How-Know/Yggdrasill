-- Fix: cycle_elapsed must represent the "current-cycle accumulated time"
-- for the group, monotonically accumulating across pause/resume until the
-- group transitions to phase=3 (submit). Previously it was only the live
-- segment since run_start, which dropped to 0 on pause and reset when the
-- student navigated away and back, causing the M5 detail screen to appear
-- to reset mid-cycle.
--
-- New definition (group-level cycle_elapsed):
--   cycle_elapsed =
--       max(active_items.cycle_elapsed_sec)      -- accumulated via items
--                                                  since last submit reset
--     + (now() - gr.run_start) if running        -- live segment while
--                                                   group runtime is phase=2
--
-- This makes the server the single source of truth and lets M5 simply seed
-- from g.cycle_elapsed without needing special local-watch baselines.
...
begin
  ...
  select
    ...
    -- New cycle_elapsed: always accumulates across pause/resume.
    -- Resets only when items transition to phase=3 (submit) which zeros
    -- accumulated_ms - cycle_base_accumulated_ms via homework_submit.
    (
      gs.cycle_elapsed
      + case
          when gr.phase = 2 and gr.run_start is not null
