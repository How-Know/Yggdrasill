-- 20260212100000: hard cleanup for previous student-level experiment

-- 1) remove expanded stats RPC signature if present
...
-- 2) rebuild baseline unit-stats objects (drop old then recreate)
drop function if exists public.homework_unit_stats(
  uuid,
  uuid,
  text,
...
-- 3) cleanup old experimental columns/indexes
drop index if exists idx_students_academy_grade_levels;
drop index if exists idx_homework_items_snapshot_filter;

alter table if exists public.students
  drop column if exists current_level_code,
  drop column if exists target_level_code;

alter table if exists public.homework_items
  drop column if exists student_education_level_snapshot,
  drop column if exists student_grade_snapshot,
  drop column if exists student_current_level_snapshot,
  drop column if exists student_target_level_snapshot;

-- 4) cleanup experimental tables
drop table if exists public.student_level_states cascade;
drop table if exists public.student_level_scales cascade;

-- 5) restore baseline completed-only unit stats view/function
...
revoke all on function public.homework_unit_stats(
  uuid,
  uuid,
  text,
  text,
...
grant execute on function public.homework_unit_stats(
  uuid,
  uuid,
  text,
  text,
