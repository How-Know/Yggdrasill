-- 완료 개수/전체 개수는 스냅샷 item_ids 전체를 기준으로 집계한다.
-- (완료 후 m5 목록에서 빠져 snap_plan_items 필터에서 사라지던 문제 수정)
-- % 분자의 잔여분 집계는 기존 snap_plan_items 경로를 유지한다.
...
begin
  ...
  if v_snapshot_at is not null
    ...
    -- 개수용: 스냅샷에 있던 과제 그룹 전부 (완료로 목록에서 빠져도 유지).
    count_by_group as (
      select
    ...
    snap_plan_items as (
      ...
      -- 완료되어 목록에서 빠져도 잔여분 집계에 남겨 %,개수와 어긋나지 않게 한다.
      or exists (
        select 1
    ...
    where v_attendance_id is not null
      ...
      and exists (
        ...
        where ar.id = v_attendance_id
          and h.completed_at >= ar.arrival_time
      )
  ),
  by_group as (
