-- 교재 카드: 수정률용 집계 추가
--   first_wrong_count  = 최초 시도가 틀린 문항 수
--   corrected_count    = 그중 현재 정답인 문항 수
-- 수정률 = corrected_count / first_wrong_count
...
begin
  ...
  book_stats as (
    select
      ...
      -- 최초 오답
      count(*) filter (
        where m.record_id is not null and not m.first_attempt_correct
      ) as first_wrong_count,
      -- 최초 오답 → 현재 정답 (수정 성공)
      count(*) filter (
        where m.record_id is not null
          and not m.first_attempt_correct
          and m.is_correct
      ) as corrected_count,
