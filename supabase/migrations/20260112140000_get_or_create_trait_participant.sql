-- Get or create a survey_participants row for trait survey (WebView/anon safe).
-- We use SECURITY DEFINER to avoid RLS limitations (anon cannot select/update).
...
begin
  ...
  insert into public.survey_participants (id, survey_id, client_id, name, school, grade, level)
  values (
    p_participant_id,
    v_survey_id,
    p_client_id,
...
revoke all on function public.get_or_create_trait_participant(uuid, text, text, text, text, text, text) from public;
grant execute on function public.get_or_create_trait_participant(uuid, text, text, text, text, text, text) to anon, authenticated;
