-- 예약 과제 번들 RPC에 recommended_minutes / recommended_minutes_auto 반영.
-- 출제 직후 reload·진행 중 upsert 경로에서 권장시간이 유실되지 않게 한다.
...
begin
  ...
  loop
    ...
    values (
      ...
      case
        when (elem ? 'recommended_minutes')
          and coalesce((elem->>'recommended_minutes')::int, 0) > 0
        then (elem->>'recommended_minutes')::int
        else null
      end,
      case
        when (elem ? 'recommended_minutes_auto')
          and coalesce((elem->>'recommended_minutes_auto')::int, 0) > 0
        then (elem->>'recommended_minutes_auto')::int
        else null
      end,
      case
        when nullif(
