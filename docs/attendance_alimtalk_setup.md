...
- 지각 알림은 "수업 시작 + `student_payment_info.lateness_threshold` 분 경과 + 미도착(arrival_time is null)"일 때 큐에 적재되어 발송됩니다.
...
5. 지각 테스트: `class_date_time`을 10분 이상 지난 시간으로 두고 `arrival_time`을 비운 뒤 함수 실행
