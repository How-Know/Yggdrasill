...
기존 `TextbookSolveScreen`의 시간제한 시험 모드로 진입한다. 별도 풀이 화면을
두지 않아 교재 풀이의 필기 인식·수식 편집·연습장을 그대로 재사용한다.

학습앱에서 개념원리 시간제한 테스트를 만들 때 과제양식은 `앱`이 기본이며,
그룹 제목을 직접 수정하지 않았다면 제한시간 입력에 맞춰 `20분 테스트`처럼
자동 갱신한다. 초기 V0에서 `프린트`로 잘못 저장된 기존 테스트도 학생앱은
테스트 플래그와 제한시간을 기준으로 시험 모드를 열어 복구한다.

1. 제한시간과 1회 응시 안내
2. 첫 문항 메타데이터·렌더 PDF 준비
3. 첫 문항 준비 완료 후 서버 세션 시작 또는 재개
4. 이전 attempt를 확인해 다음 미응답 위치 복원
5. 문항 표시 후 exposure 기록
6. active time과 wall time 측정
7. 자동채점 제출 또는 패스
8. 다음 문항으로 단방향 이동
9. 전 문항 소진 또는 서버 마감 시 세션 종료
10. 정답·오답·패스·timeout·노출·정확도 요약

응시 화면은 문항보기만 제공하고 본문·정답·해설 이동은 제공하지 않는다.
정답 카드에는 원본 번호나 출처 대신 현재 시험 순번과 남은 시간을 표시하고,
모드 전환 버튼 자리에는 현재까지 제출한 문항 수를 표시한다. 답을 제출하지
않고 `다음 문제`를 누르면 원자료는 `skipped`로 보존하되 점수에서는 오답으로
처리하며, 제출 후에는 이전 문항으로 돌아갈 수 없다.
