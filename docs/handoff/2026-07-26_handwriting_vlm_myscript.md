...
> ⚠️ **이 문서는 MyScript 도입 이전 시점 기준이다.** 3장의 "맥에서 할 일"은
> 이미 완료되어 MyScript iink 4.5 가 통합돼 있다. 필기 인식 관련 최신 상태와
> 다음 작업은 `2026-08-12_set_notation_myscript_sk.md` 를 볼 것.
> 이 문서는 VLM 폴백·동치 채점 로그의 설계 배경 기록으로만 유효하다.
