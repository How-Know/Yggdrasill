# 점수화/집계 규칙

이 문서는 점수 체계를 "한 번에 완성"하지 않고, **마이그레이션처럼 누적 개선**하기 위한 기준서다.
각 점수 추가/수정은 반드시 변경 로그(마이그레이션 문서)와 함께 관리한다.

## 1) 운영 원칙

- 점수는 평가 그 자체가 아니라 **해석 보조 신호**다.
- 개입 가능 변수는 단계적으로 확장하되, **전체 조화(밸런스)**를 유지한다.
- 비개입 변수는 당장 총점 반영보다 **관찰/설명 축**으로 먼저 관리한다.
- 한 번에 여러 축을 크게 바꾸지 않고, **작은 단위 변경 + 검증**으로 진행한다.

## 2) 점수 스키마(공통)

새 지표를 추가할 때 아래 항목을 같이 정의한다.

- `id`: 시스템 내부 식별자(예: `attendance_score_v1`)
- `label`: UI 표기명(예: `출석 점수`)
- `bucket`: `non_intervention` | `intervention` | `performance`
- `scale`: 기본 `0~100`
- `direction`: 높을수록 좋은지/나쁜지 (`higher_is_better` 등)
- `formula_version`: 계산식 버전
- `weight`: 동일 버킷 내 가중치(총합 규칙 적용)
- `data_requirements`: 필요 데이터와 최소 건수
- `fallback_rule`: 데이터 부족 시 처리 방식

## 3) 비율/조화 가드레일

변경 시 아래 가드레일을 지킨다.

- **단일 지표 과대지배 방지**: 한 지표가 버킷 점수의 대부분을 결정하지 않도록 제한(권장 상한 35%).
- **결측 강건성**: 일부 지표가 비어도 전체가 붕괴하지 않게 `유효 가중치 재정규화` 적용.
- **신규 학생 보호**: 표본 적을 때 prior/k 스무딩 또는 최소 표본 규칙 사용.
- **시간 민감 지표**: 최근성 반영이 필요한 경우 반감기/감쇠 함수 명시.
- **비교 공정성**: 수업량·재원기간 편향을 줄이기 위해 비율형 집계 우선.

## 4) 집계 규칙(권장 기본)

### 4-1. 지표 점수

- 각 지표는 0~100으로 정규화해 노출한다.
- 계산 불가 시 `null` 대신 상태를 명시한다(예: `기록 부족`).

### 4-2. 버킷 점수

- 버킷 점수 = `Σ(지표점수 * 가중치) / Σ(유효 가중치)`
- 유효 가중치 = 현재 계산 가능한 지표의 가중치 합

### 4-3. 총점(향후)

- 총점은 후속 단계에서 도입.
- 도입 시에도 버킷별 비중/책임 범위를 문서로 먼저 확정 후 구현한다.

## 5) 마이그레이션 프로세스

점수 변경은 아래 순서로 진행한다.

1. 변경 제안 작성(무엇/왜/영향 범위)
2. 마이그레이션 문서 추가(`docs/assessment/scoring_migrations/`)
3. 코드 반영(서비스 계산식 + UI 표기)
4. 검증(편향/감쇠/렌더링/린트)
5. 필요 시 롤백 절차 문서화

## 6) 마이그레이션 문서 규격

파일명 규칙:

- `YYYYMMDD_NNN_<slug>.md`
- 예: `20260212_001_attendance_score_v1.md`

필수 섹션:

- 목표/배경
- 변경 전/후 규칙
- 파라미터(상수)와 근거
- 영향 범위(서비스, UI, 데이터)
- 검증 체크리스트
- 롤백 기준

## 7) 현재 적용 상태(2026-07-30)

- 개입 가능 변수:
  - `출석 점수 v1` 적용
  - 상세는 `scoring_migrations/20260212_001_attendance_score_v1.md` 참조
  - `출석 점수 v2(보강+순위)` 적용
  - 상세는 `scoring_migrations/20260212_002_attendance_makeup_rank_v2.md` 참조
  - `출석 점수 v3(과거 planned 미기록=결석)` 적용
  - 상세는 `scoring_migrations/20260730_005_attendance_past_planned_absent_v3.md` 참조
  - `출석 점수 v4(결석 > 보강 > 지각)` 적용
  - 상세는 `scoring_migrations/20260730_006_attendance_priority_v4.md` 참조
  - `출석 점수 v4.1(가중 유효 표본 8회)` 적용
  - 상세는 `scoring_migrations/20260730_007_attendance_evidence_v4_1.md` 참조
  - `과제 점수 v1(EXP 누적+약한 희석+순위)` 적용
  - 상세는 `scoring_migrations/20260213_004_homework_score_v1.md` 참조
- 총점/자동 반영 파이프라인:
  - 아직 미도입(2단계 이후)
- 스냅샷 정책(출석/과제):
  - 상세는 `scoring_snapshot_policy.md` 및
    `scoring_migrations/20260213_003_snapshot_policy_attendance_homework_v1.md` 참조

## 8) 추가 원칙

- 비개입 변수도 장기적으로 수치화를 목표로 한다(객관적이지 않아도 됨).

## 9) 출석 점수 v4 규칙 요약

### 9-0. v3 포함 규칙 (과거 planned)

- 오늘 이전 + 순수 planned 미기록(`is_planned=true`, 미등원) → **결석(0.0)** 으로 점수 반영
- 오늘·미래 planned 미처리만 `pendingIgnored`로 제외
- UI `judgeAttendanceResult`의 과거 planned=결석과 동일 기준(날짜 단위)

### 9-1. 감점 계산

- 모든 비율은 28일 반감기 가중치를 사용한다.
- 결석 감점: `min(결석률 × 140, 70점)`
- 보강 감점: `min(보강률 × 40, 20점)`
- 지각 감점: `min(지각률 × 10, 10점)`
- 최종 점수: `100 - 결석 감점 - 보강 감점 - 지각 감점`

### 9-2. 순위 정책

- 결석 구간 우선: A(0~5%) / B(5~10%) / C(10~20%) / D(20~30%) / E(30% 이상)
- 같은 구간에서는 점수 내림차순, 보강률 오름차순, 지각률 오름차순
- 결석 구간이 다르면 보강·지각으로 순위가 뒤집히지 않는다.

### 9-3. 표본 부족·참여량

- 가중 유효 수업 8회 미만은 표본 부족으로 표시한다.
- 순위 계산 시 8회까지 부족한 분량을 학원 평균으로 채운다.
- 8회 이상은 개인 기록만 사용한다.
- 최근 28일 수업 수의 주당 평균은 `학습 참여량`으로 별도 표시하고 감점에는 직접 사용하지 않는다.

## 10) 스냅샷 정책 경계(현재 단계)

### 10-1. 우선 적용 범위

- 지금 단계 스냅샷 대상은 `출석 점수`, `과제 점수` 두 축만 포함한다.
- 문제 단위 세부 분석(문항 난이도/유형/오답 패턴 기반 모델링)은 최종 단계로 미룬다.

### 10-2. 참여 규칙

- 레벨 미입력 학생은 스냅샷 계산에서 제외한다.
- 제외 학생은 UI에서 실시간 현재 상태만 표시한다.
- 최소 참여 조건은 `현재 등급 + 예상 등급`이 유효한 경우로 본다.
  - (`희망 등급`은 비교축 보강용으로 저장하되, 미입력이어도 스냅샷 참여는 허용)

### 10-3. 주기 분리 원칙

- 출석과 과제는 이벤트 밀도가 다르므로 스냅샷 주기를 분리한다.
- 권장 기준:
  - 출석: 학생-수업일 단위(저빈도)
  - 과제: 학생-일 단위 집계(고빈도 이벤트 압축)

### 10-4. 비교 축

- 현재 위치: 같은 학년 코호트 내 상대 위치
- 예상 위치: 고3 기준 코호트 내 상대 위치(현재는 수동 예상등급 기반)
- 희망등급 달성자 기준: 과거 달성자 분포 대비 현재 진행 수준

### 10-5. 상세 문서

- 운영 상세/볼륨 추정/권장 파라미터는 `scoring_snapshot_policy.md`를 단일 기준서로 사용한다.
